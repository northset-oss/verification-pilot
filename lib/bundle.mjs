...
import { validateEconomicIdentity } from './economic-identity.mjs';
...
const WORKSPACE_EVIDENCE_FIELDS = [
  'workspace_write_allowlist', 'workspace_file_count_limit',
  'workspace_bytes_limit', 'initial_workspace_manifest_digest',
  'post_run_changed_tracked_paths', 'post_run_untracked_paths', 'post_run_mode_changes',
];
const WORKSPACE_ENVIRONMENT_FIELDS = new Set(['workspace_mode', ...WORKSPACE_EVIDENCE_FIELDS]);
const OPTIONAL_BUNDLE_FILES = ['issue_snapshot.json', 'patch.diff', 'ci_links.json', 'economic.json'];
...
export function validateRunRecord(runRecord) {
  ...
  if (Object.hasOwn(runRecord, 'schema_version') && ![1, 2].includes(runRecord.schema_version)) {
    addError(errors, 'RUN_RECORD_VERSION', '$.schema_version', 'must equal 1 or 2');
  }
  ...
  if (Object.hasOwn(runRecord, 'environment') && requireType(errors, runRecord.environment, 'object', '$.environment')) {
    for (const key of Object.keys(runRecord.environment).sort()) {
      if (!ENVIRONMENT_FIELDS.has(key) && !WORKSPACE_ENVIRONMENT_FIELDS.has(key)) {
    ...
    if (WORKSPACE_EVIDENCE_FIELDS.some((field) => Object.hasOwn(runRecord.environment, field))) {
      requireField(errors, runRecord.environment, 'workspace_mode', '$.environment');
    }
    if (Object.hasOwn(runRecord.environment, 'workspace_mode')) {
      const value = runRecord.environment.workspace_mode;
      if (requireType(errors, value, 'string', '$.environment.workspace_mode') &&
          !['readonly', 'writable_copy'].includes(value)) {
        addError(errors, 'RUN_RECORD_FORMAT', '$.environment.workspace_mode', 'must be readonly or writable_copy');
      }
      for (const field of WORKSPACE_EVIDENCE_FIELDS) {
        requireField(errors, runRecord.environment, field, '$.environment');
      }
    }
    if (Object.hasOwn(runRecord.environment, 'initial_workspace_manifest_digest')) {
      const value = runRecord.environment.initial_workspace_manifest_digest;
      if (requireType(errors, value, 'string', '$.environment.initial_workspace_manifest_digest') &&
          !BUNDLE_DIGEST_PATTERN.test(value)) {
        addError(errors, 'RUN_RECORD_FORMAT', '$.environment.initial_workspace_manifest_digest', 'must be sha256: followed by 64 hex characters');
      }
    }
    for (const field of ['workspace_write_allowlist', 'post_run_changed_tracked_paths', 'post_run_untracked_paths', 'post_run_mode_changes']) {
      if (Object.hasOwn(runRecord.environment, field)) {
        const value = runRecord.environment[field];
        if (requireType(errors, value, 'array', `$.environment.${field}`)) {
          value.forEach((item, index) => requireType(errors, item, 'string', `$.environment.${field}[${index}]`));
        }
      }
    }
    if (Array.isArray(runRecord.environment.workspace_write_allowlist)) {
      const allowlist = runRecord.environment.workspace_write_allowlist;
      if (allowlist.length > 32) {
        addError(errors, 'RUN_RECORD_RANGE', '$.environment.workspace_write_allowlist', 'must contain at most 32 paths');
      }
      allowlist.forEach((item, index) => {
        if (typeof item !== 'string') return;
        if (item.length === 0 || item !== path.posix.normalize(item) ||
            path.posix.isAbsolute(item) || item.startsWith('../')) {
          addError(errors, 'RUN_RECORD_FORMAT', `$.environment.workspace_write_allowlist[${index}]`, 'must be a normalized relative path');
        }
      });
      if (runRecord.environment.workspace_mode === 'readonly' && allowlist.length > 0) {
        addError(errors, 'RUN_RECORD_FORMAT', '$.environment.workspace_write_allowlist', 'must be empty when workspace_mode is readonly');
      }
    }
    for (const field of ['workspace_file_count_limit', 'workspace_bytes_limit']) {
      if (Object.hasOwn(runRecord.environment, field)) {
        const value = runRecord.environment[field];
        if (requireType(errors, value, 'integer', `$.environment.${field}`) && value <= 0) {
          addError(errors, 'RUN_RECORD_RANGE', `$.environment.${field}`, 'must be positive');
        }
      }
    }
  ...
  if (runRecord.schema_version === 2) {
    requireField(errors, runRecord, 'usage', '$');
    if (Object.hasOwn(runRecord, 'usage') && requireType(errors, runRecord.usage, 'object', '$.usage')) {
      const fields = ['networked_setup_elapsed_ms', 'dependency_install_ms', 'declared_commands_ms', 'cpu_ms', 'peak_rss_bytes'];
      for (const key of Object.keys(runRecord.usage).sort()) {
        if (!fields.includes(key)) addError(errors, 'RUN_RECORD_ADDITIONAL_PROPERTY', `$.usage.${key}`, 'is not an allowed property');
      }
      for (const field of fields) requireField(errors, runRecord.usage, field, '$.usage');
      for (const field of fields) {
        if (!Object.hasOwn(runRecord.usage, field)) continue;
        const nullable = field === 'dependency_install_ms' || field === 'cpu_ms' || field === 'peak_rss_bytes';
        if (requireType(errors, runRecord.usage[field], 'integer', `$.usage.${field}`, {nullable})
          && runRecord.usage[field] !== null && runRecord.usage[field] < 0) {
          addError(errors, 'RUN_RECORD_RANGE', `$.usage.${field}`, 'must be non-negative');
        }
      }
    }
  }
...
async function semanticIssues(missionDirectory, bundleDirectory) {
  ...
  try {
    const economic = JSON.parse(await readFile(path.join(bundleDirectory, 'economic.json'), 'utf8'));
    validateEconomicIdentity(economic, {mission, runRecord});
  } catch (error) {
    if (error.code !== 'ENOENT') issue('economic.json', error.message);
  }
  return issues;
}
