...
const VARIANTS = new Set(['own_repo_rehearsal', 'V', 'W', 'F']);
const CLAIMS_TIERS = new Set(['R0', 'R1', 'R2', 'R3']);
...
const TIER_LANGUAGE_PHRASES = ['on-chain', 'settlement'];
...
function checkUri(errors, value, path) {
  if (!isUri(value)) {
    addError(errors, 'STRUCTURE_FORMAT', path, 'must be an absolute URL');
    return false;
  }
  return true;
}
...
function validateStructural(receipt, errors) {
  ...
  if (Object.hasOwn(receipt, 'claims_tier')) {
    if (validateStringArray(errors, receipt.claims_tier, '$.claims_tier', { minItems: 1, unique: true })) {
...
function validatePolicy(receipt, errors) {
  if (
    ['V', 'W', 'F'].includes(receipt.variant) &&
  ...
  if (Array.isArray(receipt.claims_tier) && receipt.claims_tier.includes('R3') && receipt.variant !== 'own_repo_rehearsal') {
    addError(
      errors,
      'R3_OWN_REPO_ONLY',
      '$.claims_tier',
      'R3 claims are permitted only for own_repo_rehearsal receipts',
    );
  }
  ...
  if (!Array.isArray(receipt.claims_tier) || !receipt.claims_tier.includes('R3')) {
    for (const { path, value } of freeText) {
      const normalized = value.toLowerCase();
      for (const phrase of TIER_LANGUAGE_PHRASES) {
        if (normalized.includes(phrase)) {
          addError(
            ...
            'TIER_LANGUAGE',
            path,
            `contains "${phrase}", which requires an R3 claim`,
          );
        }
      }
    }
