...
function assertWorkspaceModeMatch(executorConfig, mission) {
  if (mission.workspace_mode === undefined || mission.workspace_mode === executorConfig.workspace_mode) return;
  throw new PipelineError('mission workspace_mode must equal executor.workspace_mode', [
    pipelineIssue(
      'WORKSPACE_MODE_MISMATCH',
      '$.mission.workspace_mode',
      'mission workspace_mode must equal the effective executor workspace_mode',
    ),
  ]);
}
...
export async function runPipeline(input, options) {
  ...
  try {
    ...
    assertWorkspaceModeMatch(executorConfig, input.mission);
