...
// The executor deliberately records failures and continues (a run record may honestly say "it
// failed"). A mission being published as a SUCCESSFUL receipt must opt into this gate so a
// failed or timed-out command can never reach the bundle/ledger/attestation path.
function assertCommandsSucceeded(execution) {
  ...
  throw new PipelineError('declared commands must all succeed under --require-success', [
    pipelineIssue(
      'COMMAND_FAILED',
      '$.mission.commands_declared',
      failed
 ...
 *
 * @param {unknown} input
 * @param {{missionsDir: string, siteFile?: string, now?: string, force?: boolean, requireSuccess?: boolean, executeImpl?: typeof execute, ledgerImpl?: typeof buildLedger, renderImpl?: typeof renderLedger, onWarning?: (message: string) => void}} options
...
export async function runPipeline(input, options) {
  ...
  try {
    ...
    const execution = await executeImpl(executorConfig, { outDir: executionDir, now: timestamp });
    assertExecutedCommandsMatch(execution, input.mission);
    if (options.requireSuccess === true) assertCommandsSucceeded(execution);
    ...
    // Stage the rendered site page BEFORE either rename so a render failure rolls the whole
    // mission back and never leaves index.json and the public page disagreeing. Staging lives
    // next to the target so each rename is same-directory (atomic, no cross-device copy).
    if (siteFile !== null) {
      siteStaging = path.join(
        path.dirname(siteFile),
        `.pipeline-site-${input.mission.mission_id}-${path.basename(siteFile)}`,
      );
    ...
    ledgerTemp = null;
    if (siteStaging !== null) {
      await rename(siteStaging, siteFile);
      siteStaging = null;
    }
