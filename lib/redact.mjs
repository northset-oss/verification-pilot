...
function isJwtCandidate(value) {
  ...
  try {
    ...
    return decodedHeader !== null && typeof decodedHeader === 'object' && !Array.isArray(decodedHeader)
      && typeof decodedHeader.alg === 'string'
      && decodedPayload !== null && typeof decodedPayload === 'object' && !Array.isArray(decodedPayload);
  } catch {
    return false;
  }
}
...
function redactLine(line, redactions) {
  ...
  redacted = redacted.replace(
    ...
    (candidate) => (isJwtCandidate(candidate) ? count(redactions, 'jwt') : candidate),
  );
  redacted = replaceMatches(
    redacted,
  ...
  redacted = redacted.replace(EMAIL_PATTERN, (email) => {
    ...
    const topLevelLabel = email.slice(email.lastIndexOf('.') + 1);
    // Numeric final labels are package versions such as `pkg@1.2.3`, not DNS email domains.
    if (/^\d+$/.test(topLevelLabel)) return email;
