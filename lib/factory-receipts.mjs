...
const FACTORY_ATTESTATION_PATH = '/repos/northset-oss/verification-pilot/attestations/';
...
function validateStructuredProof(proof, missionId, commitOid) {
  ...
  const currentRequired = proof.schema_version === 3
    ? [...required, 'receipt_visibility', 'consent_scopes']
    : required;
  if (!exactKeys(proof, currentRequired) || ![2, 3].includes(proof.schema_version)) {
    throw new TypeError(`${missionId} structured proof.json has an invalid shape`);
  }
  if (proof.schema_version === 3) {
    if (!['private_internal', 'public_opt_in'].includes(proof.receipt_visibility)) {
      throw new TypeError(`${missionId} receipt_visibility must be private_internal or public_opt_in`);
    }
    proof.consent_scopes = validateConsentScopes(proof.consent_scopes, missionId);
    const publicationGranted =
      proof.consent_scopes.scopes.receipt_publication_consent.status === 'granted';
    if (proof.receipt_visibility === 'public_opt_in' && !publicationGranted) {
      throw new TypeError(`${missionId} public_opt_in requires receipt publication consent`);
    }
    if (proof.receipt_visibility === 'private_internal' && publicationGranted) {
      throw new TypeError(`${missionId} private_internal cannot claim receipt publication consent`);
    }
  }
  validateCommonProof(proof, missionId, commitOid);
...
function validateProof(proof, missionId, commitOid) {
  ...
  if ([2, 3].includes(proof.schema_version)) return validateStructuredProof(proof, missionId, commitOid);
...
function validateFactoryPublication(value, missionId, commitOid, proofSha256, repository) {
  const requiredV1 = [
  ...
  const required = value?.schema_version === 2
    ? [...requiredV1, 'pr_head_oid', 'merge_commit_oid'] : requiredV1;
  if (!exactKeys(value, required) || ![1, 2].includes(value.schema_version) || value.mission_id !== missionId) {
  ...
  const expectedPrUrl = `https://github.com/${repository}/pull/${value.pr_number}`;
  if (value.pr_url !== expectedPrUrl) {
    throw new TypeError(`${missionId} publication pr_url must exactly match ${expectedPrUrl}`);
  }
  ...
  if (value.schema_version === 2) {
    validOid(value.pr_head_oid, `${missionId} publication pr_head_oid`);
    if (value.merge_commit_oid !== null) {
      validOid(value.merge_commit_oid, `${missionId} publication merge_commit_oid`);
    }
    if (value.merged !== (value.merge_commit_oid !== null)) {
      throw new TypeError(`${missionId} publication merge_commit_oid is inconsistent`);
    }
  }
  ...
  const attestationUrl = validHttpsUrl(
    value.attestation_url, `${missionId} publication attestation_url`, {nullable: true});
  if (value.attestation_state === 'RECEIPT_ATTESTED') {
    if (attestationUrl === null) {
      throw new TypeError(`${missionId} attested publication needs an attestation URL`);
    }
    const url = new URL(attestationUrl);
    const attestationPath = decodeURIComponent(url.pathname);
    if (url.host !== 'api.github.com' ||
        attestationPath !== `${FACTORY_ATTESTATION_PATH}${proofSha256}`) {
      throw new TypeError(`${missionId} publication attestation URL does not bind the current proof digest`);
    }
  } else if (attestationUrl !== null) {
    throw new TypeError(`${missionId} unattested publication cannot include an attestation URL`);
  }
  validTime(value.observed_at, `${missionId} publication observed_at`);
  return value;
}

async function publicationFor(missionDirectory, missionId, commitOid, proofSha256, repository) {
  ...
  try {
    ...
    return {bytes, value: validateFactoryPublication(
      value, missionId, commitOid, proofSha256, repository)};
  } catch (error) {
...
function canonicalPublication(publicationRecord) {
  ...
  return {
    ...
    pr_url: value.pr_url,
    pr_head_oid: value.pr_head_oid ?? value.contribution_commit_oid,
    base_branch: null,
    head_drift: (value.pr_head_oid ?? value.contribution_commit_oid) !== value.contribution_commit_oid,
    ci_state: value.ci_state === null ? null : value.ci_state.toLowerCase(),
    merge_commit_oid: value.merge_commit_oid ?? null,
...
function baseReceipt(proof, proofSha256, rawProofUrl, publicationRecord) {
  ...
  const consentScopes = proof.schema_version === 3
    ? proof.consent_scopes
    : absentConsentScopes(missionId);
  return {
    version: 3,
    mission_id: missionId,
    ...
    public_listing: proof.schema_version === 3 && proof.receipt_visibility === 'public_opt_in'
      ? publicListingFor({publication, consent: consentScopes})
      : 'private_internal',
    marketing_reference: false,
    evidence_classification: 'CURRENT_RECEIPT_RECORD',
...
function projectFactoryProof(proof, proofSha256, rawProofUrl, publicationRecord) {
  return [2, 3].includes(proof.schema_version)
...
export async function selectFactoryProofAttestationSubjects({
  ...
  for (const sourcePath of added) {
    ...
    const validatedProof = validateProof(proof, missionId, commitOid);
    if (
      validatedProof.schema_version !== 3
      || validatedProof.receipt_visibility !== 'public_opt_in'
      || !receiptPublicationGranted(validatedProof.consent_scopes)
    ) {
      continue;
    }
...
export async function mergeFactoryReceipts({
  ...
  if (!isObject(index) || index.version !== '1' || !Array.isArray(index.missions)) {
    throw new TypeError('canonical ledger index is invalid');
  }
  validTime(index.generated_at, 'canonical ledger generated_at');
  let generatedAt = index.generated_at;
  ...
  for (const entry of missionEntries) {
    ...
    const publicationRecord = await publicationFor(
      missionDirectory, missionId, commitOid, pointer.proof_sha256, proof.repository);
    if (publicationRecord !== null &&
        Date.parse(publicationRecord.value.observed_at) > Date.parse(generatedAt)) {
      generatedAt = publicationRecord.value.observed_at;
    }
