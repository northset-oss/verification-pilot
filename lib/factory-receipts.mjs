...
function validateFactoryPublication(value, missionId, commitOid, proofSha256) {
  const requiredV1 = [
...
async function publicationFor(missionDirectory, missionId, commitOid, proofSha256) {
  ...
  try {
    ...
    return {bytes, value: validateFactoryPublication(value, missionId, commitOid, proofSha256)};
  } catch (error) {
...
export async function mergeFactoryReceipts({
  ...
  for (const entry of missionEntries) {
    ...
    const publicationRecord = await publicationFor(
      missionDirectory, missionId, commitOid, pointer.proof_sha256);
