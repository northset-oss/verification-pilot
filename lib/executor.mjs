...
function runGit(gitImpl, args) {
  let child;
  try {
    child = gitImpl('git', args, { stdio: ['ignore', 'pipe', 'ignore'] });
  } catch {
...
async function treeDigest(root, excludeNames) {
  ...
  for (const file of files) hash.update(`${file.kind}\0${file.mode}\0${file.path}\0${file.hash}\n`);
...
export async function execute(configValue, options) {
  ...
  try {
    ...
    if (config.patch_file !== null) {
      ...
      await writeFile(path.join(workspaceDir, patchName), patchBytes);
