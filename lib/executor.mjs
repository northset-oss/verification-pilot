...
const OPTIONAL_CONFIG_FIELDS = new Set(['profile', 'workspace_mode', 'workspace_write_allowlist']);
...
const PROFILES = {
  ...
  go: {
    path: `/workspace/.northset/go/bin:/usr/local/go/bin:${BASE_PATH}`,
    setup: ['test -x /usr/local/go/bin/go'],
    environment: [
      'GOCACHE=/tmp/northset-go-build-cache',
      'GOMODCACHE=/workspace/.northset/go-mod-cache',
      'GOPATH=/workspace/.northset/go',
    ],
    cacheDirectories: ['go', 'go-mod-cache'],
  },
  rust: {
    path: `/workspace/.northset/cargo/bin:/usr/local/cargo/bin:${BASE_PATH}`,
    setup: ['test -x /usr/local/cargo/bin/cargo && test -x /usr/local/cargo/bin/rustc'],
    environment: [
      'CARGO_HOME=/workspace/.northset/cargo',
      'CARGO_TARGET_DIR=/workspace/.northset/cargo-target',
      'RUSTUP_HOME=/usr/local/rustup',
    ],
    cacheDirectories: ['cargo', 'cargo-target'],
  },
};
const NETWORK_POLICY = 'phaseA:bridge,phaseB:none';
const TERMINATION_GRACE_MS = 10_000;
const WORKSPACE_MONITOR_INTERVAL_MS = 250;
const TMPFS_SIZE_MB = 512;
const UTILITY_OUTPUT_LIMIT = 64 * 1024;
const WORKSPACE_MAX_BYTES = 2 * 1024 * 1024 * 1024;
const WORKSPACE_MAX_FILES = 200_000;
...
export function validateExecutorConfig(value) {
  ...
  for (const field of CONFIG_FIELDS) {
    if (OPTIONAL_CONFIG_FIELDS.has(field)) continue;
  ...
  const workspaceMode = value.workspace_mode ?? 'readonly';
  if (!['readonly', 'writable_copy'].includes(workspaceMode)) {
    errors.push(validationError('$.workspace_mode', 'must be readonly or writable_copy'));
  }
  const workspaceWriteAllowlist = value.workspace_write_allowlist ?? [];
  if (!Array.isArray(workspaceWriteAllowlist) || workspaceWriteAllowlist.length > 32 ||
      !workspaceWriteAllowlist.every((item) => typeof item === 'string' && item.length > 0 &&
        item === path.posix.normalize(item) && !path.posix.isAbsolute(item) && !item.startsWith('../'))) {
    errors.push(validationError('$.workspace_write_allowlist', 'must be at most 32 normalized relative paths'));
  }
  if (workspaceMode === 'readonly' && workspaceWriteAllowlist.length > 0) {
    errors.push(validationError('$.workspace_write_allowlist', 'is only valid for writable_copy'));
  }
  ...
  if (!isObject(value.limits)) {
    ...
    for (const field of ['workspace_file_count', 'workspace_bytes']) {
      if (Object.hasOwn(value.limits, field)) validatePositiveNumber(errors, value.limits[field], `$.limits.${field}`, {integer: true});
    }
  }
  ...
  return {
    ...
    limits: {
      ...value.limits,
      workspace_file_count: value.limits.workspace_file_count ?? WORKSPACE_MAX_FILES,
      workspace_bytes: value.limits.workspace_bytes ?? WORKSPACE_MAX_BYTES,
    },
...
function containerEnvironment(config) {
  return [
    `PATH=${PROFILES[config.profile].path}`,
    ...FIXED_ENVIRONMENT,
    ...PROFILES[config.profile].environment,
  ];
}

function commonRunArgs(config, paths, {workspaceReadonly = false} = {}) {
  ...
  return [
    ...
    '--read-only',
    '--tmpfs',
    `/tmp:rw,exec,nosuid,nodev,size=${TMPFS_SIZE_MB}m,uid=1000,gid=1000,mode=1777`,
    '--mount',
    `type=bind,source=${workspaceDir},target=/workspace${workspaceReadonly ? ',readonly' : ''}`,
    '--workdir',
    '/workspace',
    '--entrypoint',
    '/usr/bin/env',
...
export function buildDockerArgs(phase, config, paths) {
  ...
  if (phase === 'B' || phase === 'phaseB') {
    ...
    return [
      ...commonRunArgs(config, paths, {workspaceReadonly: (config.workspace_mode ?? 'readonly') === 'readonly'}),
      '--network=none',
      paths.image ?? config.image,
      '-i',
      ...containerEnvironment(config),
...
function runDocker(
  ...
  args,
  {
    outputLimit = UTILITY_OUTPUT_LIMIT,
    timeoutMs = null,
    containerName = null,
  ...
  return new Promise((resolve, reject) => {
    ...
    const stopContainer = () => {
      if (containerName !== null) {
        try {
          ...
          // Container cleanup is best-effort; client signals remain the fallback.
        }
      }
      child.kill('SIGTERM');
    ...
    if (workspaceMonitor !== null) {
      monitorInterval = setInterval(async () => {
        if (settled || monitorRunning || monitorError !== null) return;
        monitorRunning = true;
        try {
          await workspaceMonitor();
        } catch (error) {
          if (settled) return;
          monitorError = error instanceof ExecutorError
            ? error
            : new ExecutorError(`workspace monitor failed: ${error instanceof Error ? error.message : String(error)}`);
          stopContainer();
        } finally {
...
async function treeFileCount(directory, stopAfter = Number.POSITIVE_INFINITY) {
  const stats = await lstat(directory).catch((error) => {
    if (error.code === 'ENOENT') return null;
    throw error;
  });
  if (stats === null) return 0;
  if (stats.isSymbolicLink() || !stats.isDirectory()) return 1;
  const entries = await readdir(directory).catch((error) => {
    if (error.code === 'ENOENT') return [];
    throw error;
  });
  let total = 1;
  for (const entry of entries) {
    total += await treeFileCount(path.join(directory, entry), stopAfter - total);
    if (total > stopAfter) break;
  }
  return total;
}

async function enforceWorkspaceSize(workspaceDir, limits = {}) {
  const maximumBytes = limits.workspace_bytes ?? WORKSPACE_MAX_BYTES;
  const maximumFiles = limits.workspace_file_count ?? WORKSPACE_MAX_FILES;
  if (await treeFileSize(workspaceDir, maximumBytes) > maximumBytes) {
    throw new ExecutorError('workspace exceeded size cap');
  }
  if (await treeFileCount(workspaceDir, maximumFiles) > maximumFiles) {
    throw new ExecutorError('workspace exceeded file-count cap');
  }
}
...
function cacheDirectories(profile) {
  return [...CACHE_DIRECTORIES, ...PROFILES[profile].cacheDirectories];
}

async function prepareWorkspaceCacheRoot(workspaceDir, profile) {
  ...
  for (const directory of cacheDirectories(profile)) await mkdir(path.join(cacheRoot, directory));
  await makeTreeWritable(cacheRoot);
}

async function assertWorkspaceCacheRoots(workspaceDir, profile) {
  const workspaceRealPath = await realpath(workspaceDir);
  for (const directory of cacheDirectories(profile)) {
...
async function treeManifest(root, excludeNames) {
  const files = [];
...
async function treeDigest(root, excludeNames) {
  return manifestDigest(await treeManifest(root, excludeNames));
}

function pathAllowed(file, allowlist) {
  return allowlist.some((allowed) => file === allowed || file.startsWith(`${allowed}/`));
}

function finalWorkspaceChanges(initialManifest, finalManifest, trackedFiles, allowlist) {
  const initial = new Map(initialManifest.map((entry) => [entry.path, entry]));
  const final = new Map(finalManifest.map((entry) => [entry.path, entry]));
  ...
  for (const file of trackedFiles ?? []) {
    ...
    if (!before || !after || before.kind !== after.kind || before.hash !== after.hash) changedTracked.push(file);
    if (before && after && before.mode !== after.mode) modeChanges.push(file);
  }
  const untracked = [...new Set([...initial.keys(), ...final.keys()])]
    .filter((file) => {
      if (tracked.has(file)) return false;
      const before = initial.get(file);
      const after = final.get(file);
      return !before || !after || before.kind !== after.kind || before.mode !== after.mode || before.hash !== after.hash;
    })
    .sort();
  const forbiddenUntracked = untracked.filter((file) => !pathAllowed(file, allowlist));
  const unsafeSymlinks = finalManifest
    .filter((entry) => {
      if (entry.kind !== 'symlink') return false;
      const before = initial.get(entry.path);
      return !before || before.kind !== 'symlink' || before.hash !== entry.hash;
    })
    .map((entry) => entry.path).sort();
  return {
    post_run_changed_tracked_paths: [...new Set(changedTracked)].sort(),
    post_run_untracked_paths: untracked,
    post_run_mode_changes: [...new Set(modeChanges)].sort(),
    forbidden_untracked_paths: forbiddenUntracked,
    unsafe_symlink_paths: unsafeSymlinks,
  };
}

function assertAllowedWorkspaceChanges(workspaceChanges) {
  if (workspaceChanges.post_run_changed_tracked_paths.length || workspaceChanges.post_run_mode_changes.length) {
    throw new ExecutorError('declared checks modified the approved tracked tree');
  }
  if (workspaceChanges.forbidden_untracked_paths.length) {
    throw new ExecutorError(`declared checks created unapproved paths: ${workspaceChanges.forbidden_untracked_paths.join(', ')}`);
  }
  if (workspaceChanges.unsafe_symlink_paths.length) {
    throw new ExecutorError(`declared checks created symlink paths: ${workspaceChanges.unsafe_symlink_paths.join(', ')}`);
  }
}
...
export async function execute(configValue, options) {
  ...
  try {
    const sourceStats = await lstat(config.repo_dir).catch((error) => {
      throw new ExecutorError(`cannot inspect repo_dir: ${error.message}`);
    });
    if (!sourceStats.isDirectory() || sourceStats.isSymbolicLink()) {
      throw new ExecutorError('repo_dir must be a directory and not a symlink');
    }
    // Reject an already-oversized hostile checkout before duplicating it. Traversal uses lstat
    // and never follows source symlinks; the copied tree is checked again immediately afterward.
    await enforceWorkspaceSize(config.repo_dir, config.limits);
    ...
    await enforceWorkspaceSize(workspaceDir, config.limits);
    ...
    await prepareWorkspaceCacheRoot(workspaceDir, config.profile);
    ...
    const phaseAResult = await runDocker(
      ...
      buildDockerArgs('phaseA', config, phaseAPaths),
      {
        ...
        containerName: phaseAContainer,
        workspaceMonitor: () => enforceWorkspaceSize(workspaceDir, config.limits),
      },
    );
    ...
    await enforceWorkspaceSize(workspaceDir, config.limits);
    await assertWorkspaceCacheRoots(workspaceDir, config.profile);
    await removeAllGitMetadata(workspaceDir);
    ...
    const preCheckManifest = await treeManifest(workspaceDir, new Set(['.git']));
    const preCheckTreeDigest = manifestDigest(preCheckManifest);
    ...
    for (let index = 0; index < config.commands.length; index += 1) {
      ...
      const result = await runDocker(
        ...
        }),
        {
          ...
          timeoutMs: config.limits.wall_clock_seconds_per_command * 1000,
          containerName: phaseBContainers[index],
          workspaceMonitor: config.workspace_mode === 'writable_copy'
            ? () => enforceWorkspaceSize(workspaceDir, config.limits)
            : null,
        },
      );
      ...
      await assertWorkspaceCacheRoots(workspaceDir, config.profile);
      await enforceWorkspaceSize(workspaceDir, config.limits);
      // Validate after EACH command, before a later command can consume and then erase or restore
      // a forbidden mutation. Allowlisted writable-copy artifacts may intentionally cross commands;
      // tracked changes, unapproved paths, mode changes, and new symlinks may not.
      const commandManifest = await treeManifest(workspaceDir, new Set(['.git']));
      assertAllowedWorkspaceChanges(finalWorkspaceChanges(
        preCheckManifest, commandManifest, trackedFiles, config.workspace_write_allowlist,
      ));
    }
    const postCheckManifest = await treeManifest(workspaceDir, new Set(['.git']));
    const postCheckTreeDigest = manifestDigest(postCheckManifest);
    const workspaceChanges = finalWorkspaceChanges(
      preCheckManifest, postCheckManifest, trackedFiles, config.workspace_write_allowlist,
    );
    assertAllowedWorkspaceChanges(workspaceChanges);
    ...
    const runRecord = {
      ...
      environment: {
        ...
        workspace_write_allowlist: [...config.workspace_write_allowlist],
        workspace_file_count_limit: config.limits.workspace_file_count,
        workspace_bytes_limit: config.limits.workspace_bytes,
        initial_workspace_manifest_digest: preCheckTreeDigest,
        post_run_changed_tracked_paths: workspaceChanges.post_run_changed_tracked_paths,
        post_run_untracked_paths: workspaceChanges.post_run_untracked_paths,
        post_run_mode_changes: workspaceChanges.post_run_mode_changes,
      ...
      usage: {
        networked_setup_elapsed_ms: phaseAResult.durationMs,
        dependency_install_ms: null,
        declared_commands_ms: commandRecords.reduce((total, command) => total + command.duration_ms, 0),
        cpu_ms: null,
        peak_rss_bytes: null,
      },
      notes: null,
