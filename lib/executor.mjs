...
function runGit(gitImpl, args) {
  ...
  return new Promise((resolve) => {
    ...
    child.once('close', (code) => resolve({ code, stdout: Buffer.concat(chunks).toString('utf8') }));
  });
}
...
async function deriveSourceCommit(gitImpl, workspaceDir) {
  const head = await runGit(gitImpl, ['-C', workspaceDir, 'rev-parse', 'HEAD']);
  ...
  const status = await runGit(gitImpl, ['-C', workspaceDir, 'status', '--porcelain', '--untracked-files=all']);
  if (status.code !== 0 || status.stdout.trim() !== '') return null;
  return commit;
}

async function trackedTreeDigest(gitImpl, workspaceDir) {
  const listed = await runGit(gitImpl, ['-C', workspaceDir, 'ls-files', '-z']);
...
export async function execute(configValue, options) {
  ...
  try {
    ...
    if (config.patch_file !== null) {
      ...
      const applied = await runGit(gitImpl, ['-C', workspaceDir, 'apply', '--index', '--binary', '--', patchFile]);
      if (applied.code !== 0) throw new ExecutorError(`approved patch could not be applied: ${(applied.stdout || '').trim()}`);
    }
    const approvedTrackedTreeDigest = await trackedTreeDigest(gitImpl, workspaceDir);
    ...
    const installedTrackedTreeDigest = await trackedTreeDigest(gitImpl, workspaceDir);
