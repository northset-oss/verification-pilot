  ...
  <script>
    (() => {
      ...
      for (const mission of data.missions) {
        ...
        const variantCell = textCell(mission.variant);
        if (mission.variant === 'own_repo_rehearsal' && mission.disclosure_label) {
          const label = document.createElement('small');
          label.textContent = mission.disclosure_label;
          variantCell.append(document.createElement('br'), label);
        }
        row.append(variantCell);
        const tiers = Array.isArray(mission.claims_tier) ? mission.claims_tier : [];
        row.append(textCell(tiers.length > 0 ? tiers.join(', ') : '—'));
