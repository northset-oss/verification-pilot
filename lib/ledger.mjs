...
function renderV1Overview(receipt, attestationEvidence) {
  ...
  const commandLabel = `declared command${receipt.declared_checks === 1 ? '' : 's'} passed`;
  ...
  <div class="proof-hero">
    <div class="proof-result">
      <div class="proof-score">${receipt.successful_checks}/${receipt.declared_checks}</div>
      <p class="proof-statement"><strong>${escapeHtml(commandLabel)}</strong></p>
      <p>${escapeHtml(receipt.execution_summary)}</p>
    </div>
    <dl class="proof-status-rail" data-receipt-version="1">
...
function renderReceipt(receipt, { featured = false, page = false, generatedAt = null } = {}) {
  ...
  const environmentDigestRow = environmentDigest === null
    ? ''
    : `<dt>repository digest</dt><dd>${receipt.version === 1 ? renderCopyableHash(environmentDigest, 'container image digest') : `<code>${escapeHtml(environmentDigest)}</code>`}</dd>`;
  const environmentSection = `<section class="receipt-section">${receiptSectionHeading('Environment', page)}<dl class="facts"><dt>image reference</dt><dd>${escapeHtml(receipt.environment.container_image_ref)}</dd>${environmentDigestRow}${receipt.environment.container_image_id === null ? '' : `<dt>immutable image ID</dt><dd>${receipt.version === 1 ? renderCopyableHash(receipt.environment.container_image_id, 'immutable image ID') : `<code>${escapeHtml(receipt.environment.container_image_id)}</code>`}</dd>`}${receipt.environment.container_os === null ? '' : `<dt>platform</dt><dd>${escapeHtml(receipt.environment.container_os)}/${escapeHtml(receipt.environment.container_architecture)}</dd>`}<dt>network</dt><dd>${escapeHtml(receipt.environment.network_policy)}</dd></dl></section>`;
  const checksBody = `<p><strong>Execution summary</strong><br>${escapeHtml(receipt.execution_summary)}</p><ol class="commands">${commandLines}</ol>${setupDuration}${wallDuration}${receipt.version === 2 ? `<p class="total">${escapeHtml(receipt.result)}</p>` : ''}<p class="scope-note">${escapeHtml(scopeNote)}</p>`;
  ...
  return `<article class="receipt ${receiptClass(receipt)} receipt--economic${receipt.version === 1 ? ' receipt--v1' : ''}${featured ? ' receipt--featured' : ''}"${articleId}>
  ${receiptHeader}
...
function renderReceiptOgSvg(receipt) {
  ...
  <title id="og-title">${escapeHtml(receipt.mission_id)} Proof-of-Pass Receipt</title>
  ...
  <text x="104" y="409" class="score mono">${receipt.successful_checks}/${receipt.declared_checks}</text>
...
function renderReceiptPage(receipt, generatedAt) {
  ...
  const socialTitle = `${receipt.mission_id} Proof-of-Pass Receipt · ${repoLabel(receipt.target_repo)}`;
  return renderDocument({
    ...
    body: `<main class="receipt-page"><header class="page-nav">${pageActions}</header>${renderReceipt(receipt, { page: true, generatedAt })}${renderRequestRunCta()}<section class="claims receipt-page-claims" aria-labelledby="receipt-claims-boundary"><h2 id="receipt-claims-boundary">Claims boundary</h2><p>This page reports scoped proof-of-pass receipt evidence. It does not prove code quality, security, full CI coverage, production readiness, or maintainer approval. An attestation confirms bundle provenance; it does not broaden the receipt's claim.</p><p>Read the full <a href="https://github.com/northset-oss/verification-pilot/blob/main/policies/claims_boundary.md">Claims Boundary policy</a>.</p></section><footer class="site-footer receipt-page-footer">A proof-of-pass receipt records that the declared commands returned exit 0 on the named code in the named environment. Maintainer outcome is reported separately and remains fully outside Northset’s control. Source: <a href="https://github.com/northset-oss/verification-pilot">northset-oss/verification-pilot</a>.</footer></main>`,
  });
}
