...
function renderReceipt(receipt, { featured = false, page = false, generatedAt = null } = {}) {
  ...
  const evidence = receipt.version === 2
    ...
    : `${projectSection}${renderCodeLines(receipt, { page })}${environmentSection}${checksSection}${publicScopeInterpretation}${recordSection}${limitationsSection}${correction}${signedSection}`;
...
function renderReceiptPage(receipt, generatedAt) {
  const legacyPageMetadata = receipt.version === 2
    ? ''
    : `<p>Canonical receipt · ${escapeHtml(receipt.canonical_url)}</p><p>Generated at ${escapeHtml(generatedAt)}</p>`;
