...
export function validatePublication(value, missionId) {
  ...
  for (const field of ['pr_url', 'pr_head_oid', 'decision_url', 'opened_at', 'closed_at', 'updated_at',
    'correction_note', 'scope_note', 'attestation_uri', 'bundle_digest', 'release_asset_sha256', 'ci_state',
...
function renderOutcomeStub(receipt) {
  ...
  const drift = outcome.head_drift === true
    && receipt.code.tested_commit !== null
    && outcome.pr_head_oid !== null
    ? `<p class="outcome-drift">This receipt tested <code>${escapeHtml(receipt.code.tested_commit)}</code>; the upstream outcome applies to later PR head <code>${escapeHtml(outcome.pr_head_oid)}</code>.</p>`
    : '';
  return `<section class="outcome-stub" aria-label="Live upstream outcome"><p class="stub-cut">- - - detach here - - -</p><h2>Live upstream outcome</h2>${state}${drift}</section>`;
}
...
function renderLedgerHtml(index) {
  ...
  <section class="counts" aria-label="External receipt counts"><p><strong>${counts.external}</strong> External receipts</p><p><strong>${counts.attested}</strong> Attested external receipts</p><p><strong>${counts.merged}</strong> Merged upstream</p><p><strong>${counts.awaiting}</strong> Awaiting review</p></section>
...
function renderReceiptPage(receipt) {
  return renderDocument({
    ...
    body: `<main class="receipt-page"><header class="page-nav"><div class="page-actions"><a href="../../index.html">← All Proof-of-Pass Receipts</a><span class="page-action-buttons"><a class="button-link" href="receipt.json" download>Download receipt.json</a><button type="button" data-print>Print / Save receipt</button></span></div><p>Canonical receipt · ${escapeHtml(receipt.canonical_url)}</p></header>${renderReceipt(receipt, { page: true })}<section class="claims receipt-page-claims" aria-labelledby="receipt-claims-boundary"><h2 id="receipt-claims-boundary">Claims boundary</h2><p>This page reports scoped proof-of-pass receipt evidence. It does not prove code quality, security, full CI coverage, production readiness, or maintainer approval. An attestation confirms bundle provenance; it does not broaden the receipt's claim.</p><p>Read the full <a href="https://github.com/northset-oss/verification-pilot/blob/main/policies/claims_boundary.md">Claims Boundary policy</a>.</p></section><footer class="site-footer receipt-page-footer">A proof-of-pass receipt records that the declared commands returned exit 0 on the named code in the named environment. Maintainer outcome is reported separately and remains fully outside Northset’s control. Source: <a href="https://github.com/northset-oss/verification-pilot">northset-oss/verification-pilot</a>.</footer></main>`,
  });
}

function publicReceiptSummary(receipt) {
  return {
    schema_version: 1,
...
export async function renderLedger({ indexPath, out, now = null }) {
  ...
  for (const mission of index.missions) {
    ...
    await writeOutput(path.join(receiptDirectory, 'index.html'), renderReceiptPage(receipt));
    await writeOutput(
      path.join(receiptDirectory, 'receipt.json'),
      `${JSON.stringify(publicReceiptSummary(receipt), null, 2)}\n`,
    );
  }
