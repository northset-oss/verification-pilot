...
function renderEconomicOverview(receipt, {page = false} = {}) {
  if (receipt.version !== 2) return '';
  const economic = receipt.economic_identity;
  const {task, funding, authorization, attempt_lineage: lineage, usage, work_scope: work, costs, outcome} = economic;
  const upstreamState = receipt.publication?.state ?? 'prepared';
  const costHeadline = costs.total_economic_cost === null
    ? 'Not fully priced'
    : `${costs.currency ?? 'currency not recorded'} ${costs.total_economic_cost}`;
  const costLines = costs.lines.length === 0
    ? '<p>No public priced cost lines were recorded.</p>'
    : `<ul class="cost-lines">${costs.lines.map((line) => {
      const amount = line.amount === null ? 'amount unavailable' : `${line.currency ?? costs.currency ?? 'currency not recorded'} ${line.amount}`;
      const quantity = line.quantity === null ? 'quantity unavailable' : `${line.quantity} ${readableKey(line.unit)}`;
      return `<li><strong>${escapeHtml(readableKey(line.component))}</strong><span>${escapeHtml(readableKey(line.measurement_class))} · ${escapeHtml(quantity)} · ${escapeHtml(amount)}</span></li>`;
    }).join('')}</ul>`;
  ...
  const attempts = lineage.attempts.map((attempt) => `<li><code>${escapeHtml(attempt.attempt_id)}</code><span>attempt ${attempt.attempt_sequence} · ${escapeHtml(readableKey(attempt.state))}${attempt.terminal_reason_class === null ? '' : ` · ${escapeHtml(readableKey(attempt.terminal_reason_class))}`}</span></li>`).join('');
  ...
  <p class="section-kicker">ONE RECEIPT · THREE EVIDENCE LAYERS</p>
  <h2 id="economic-overview-title">At a glance</h2>
  <div class="glance-grid">
    <div class="glance-card glance-card--pass"><span>Technical</span><strong>${escapeHtml(receipt.result)}</strong><small>${work.checks_declared} declared ${work.checks_declared === 1 ? 'check' : 'checks'}</small></div>
    <div class="glance-card"><span>Upstream outcome</span><strong>${escapeHtml(outcomeLabel(upstreamState))}</strong><small>mutable external observation</small></div>
    <div class="glance-card"><span>Attempts</span><strong>${lineage.attempts_total}</strong><small>task-bound ${lineage.attempts_total === 1 ? 'attempt' : 'attempts'}</small></div>
    <div class="glance-card glance-card--cost"><span>Economic cost</span><strong>${escapeHtml(costHeadline)}</strong><small>${escapeHtml(readableKey(costs.status))} evidence</small></div>
  </div>
</section>
<section class="economic-identity" aria-labelledby="economic-identity-title">
  <div class="section-heading"><div><p class="section-kicker">ECONOMIC LAYER</p><h2 id="economic-identity-title">Economic identity</h2></div><p>Observed sponsorship, authorization, scope, usage, and outcome. No estimates or inferred value.</p></div>
  <dl class="identity-grid">
    <div><dt>Stable task</dt><dd><code>${escapeHtml(task.task_id)}</code><span>${escapeHtml(readableKey(task.work_category))} · attempt ${task.attempt_sequence}</span></dd></div>
    <div><dt>External demand</dt><dd>${link(task.external_demand.issue_url, issueLabel(task.external_demand.issue_url))}<span>${escapeHtml(readableKey(task.external_demand.invitation_type))} invitation</span></dd></div>
    <div><dt>Sponsor</dt><dd>${escapeHtml(funding.program)}<span>${escapeHtml(funding.initiative)}</span></dd></div>
    <div><dt>Authorization</dt><dd>${escapeHtml(authorization.approved_by)}<span>${escapeHtml(approvalTime)}</span></dd></div>
    <div><dt>External transfer</dt><dd>${escapeHtml(economic.external_transfers.maintainer_payment)}<span>${economic.external_transfers.merge_contingent ? 'merge contingent' : 'not merge contingent'}</span></dd></div>
    <div><dt>Observed scope</dt><dd>${work.files_changed} files · ${work.changed_lines} changed lines<span>${work.production_files} production · ${work.test_files} test files; scope, not value</span></dd></div>
  </dl>
  <div class="effort-strip" aria-label="Recorded stage durations">
    <div><span>Discovery</span><strong>${escapeHtml(durationOrUnavailable(usage.discovery.elapsed_ms))}</strong></div>
    <div><span>Qualification</span><strong>${escapeHtml(durationOrUnavailable(usage.qualification.duration_ms))}</strong></div>
    <div><span>Authoring</span><strong>${escapeHtml(durationOrUnavailable(usage.authoring.duration_ms))}</strong></div>
    <div><span>Verification</span><strong>${escapeHtml(durationOrUnavailable(usage.verification.executor_elapsed_ms))}</strong></div>
  </div>
</section>
<section class="economic-unknowns" aria-labelledby="economic-unknowns-title">
  <div><p class="section-kicker">COST COMPLETENESS</p><h2 id="economic-unknowns-title">Known, unknown, and unpriced</h2><p><strong>Known:</strong> maintainer payment was ${escapeHtml(economic.external_transfers.maintainer_payment)}. A known zero external transfer is not a zero total cost.</p><p><strong>Total economic cost:</strong> ${costs.total_economic_cost === null ? 'unavailable while components remain missing.' : escapeHtml(costHeadline)}</p></div>
  <div><h3>Missing or unpriced components</h3><ul>${missing}</ul></div>
</section>
<details class="evidence-drawer">
  <summary><span>Economic evidence</span><small>Lineage, usage, caps, and cost provenance</small></summary>
  <div class="drawer-grid">
    <section><h3>Attempt lineage</h3><ul class="attempt-list">${attempts}</ul></section>
    <section><h3>Recorded usage</h3><dl class="facts"><dt>requested reviewer model</dt><dd>${escapeHtml(usage.qualification.requested_model)}</dd><dt>actual reviewer model</dt><dd>${escapeHtml(usage.qualification.actual_model ?? 'not captured')}</dd><dt>requested author model</dt><dd>${escapeHtml(usage.authoring.requested_model)}</dd><dt>actual author model</dt><dd>${escapeHtml(usage.authoring.actual_model ?? 'not captured')}</dd><dt>networked setup phase</dt><dd>${escapeHtml(durationOrUnavailable(usage.verification.networked_setup_elapsed_ms))}</dd><dt>install-only duration</dt><dd>${escapeHtml(durationOrUnavailable(usage.verification.dependency_install_ms))}</dd><dt>declared commands</dt><dd>${escapeHtml(durationOrUnavailable(usage.verification.declared_commands_ms))}</dd><dt>unclassified executor time</dt><dd>${escapeHtml(durationOrUnavailable(usage.verification.unclassified_executor_ms))}</dd><dt>CPU time</dt><dd>${escapeHtml(durationOrUnavailable(usage.verification.cpu_ms))}</dd><dt>peak RSS</dt><dd>${usage.verification.peak_rss_bytes === null ? 'not captured' : `${escapeHtml(usage.verification.peak_rss_bytes)} bytes`}</dd></dl></section>
    <section><h3>Configured resource envelope</h3><dl class="facts"><dt>CPU cap</dt><dd>${resource.cpus}</dd><dt>memory cap</dt><dd>${resource.memory_mb} MB</dd><dt>PID cap</dt><dd>${resource.pids}</dd><dt>command wall cap</dt><dd>${resource.wall_clock_seconds_per_command}s</dd><dt>output cap / stream</dt><dd>${resource.output_bytes_per_stream} bytes</dd></dl><p class="scope-note">Configured limits are an envelope, not observed consumption.</p></section>
    <section><h3>Cost lines</h3>${costLines}<p class="scope-note">Cost status: ${escapeHtml(costs.status)}. Values without source-backed quantities and rates remain null.</p></section>
    <section><h3>Outcome facts</h3><dl class="facts"><dt>technical checks passed</dt><dd>${outcome.technical_checks_passed}</dd><dt>PR opened</dt><dd>${outcome.pr_opened}</dd><dt>CI</dt><dd>${escapeHtml(outcome.ci_state ?? 'not observed')}</dd><dt>merged</dt><dd>${outcome.merged}</dd><dt>accepted as submitted</dt><dd>${outcome.accepted_as_submitted === null ? 'not established' : outcome.accepted_as_submitted}</dd><dt>external cycle time</dt><dd>${escapeHtml(durationOrUnavailable(outcome.time_to_close_ms))}</dd><dt>released</dt><dd>${outcome.released === null ? 'not observed' : outcome.released}</dd><dt>deployed</dt><dd>${outcome.deployed === null ? 'not observed' : outcome.deployed}</dd><dt>business result</dt><dd>${escapeHtml(outcome.business_result_observed ?? 'not observed')}</dd></dl></section>
  </div>
</details>`;
}
...
function renderReceipt(receipt, { featured = false, page = false } = {}) {
  ...
  const heading = page
    ? `<h1>Proof-of-Pass Receipt — <span class="receipt-id">${escapeHtml(receipt.mission_id)}</span></h1>`
  ...
  const environmentSection = `<section class="receipt-section">${receiptSectionHeading('Environment', page)}<dl class="facts"><dt>image reference</dt><dd>${escapeHtml(receipt.environment.container_image_ref)}</dd><dt>repository digest</dt><dd><code>${escapeHtml(receipt.environment.container_image_digest ?? 'not available in this legacy record')}</code></dd>${receipt.environment.container_image_id === null ? '' : `<dt>immutable image ID</dt><dd><code>${escapeHtml(receipt.environment.container_image_id)}</code></dd>`}${receipt.environment.container_os === null ? '' : `<dt>platform</dt><dd>${escapeHtml(receipt.environment.container_os)}/${escapeHtml(receipt.environment.container_architecture)}</dd>`}<dt>network</dt><dd>${escapeHtml(receipt.environment.network_policy)}</dd></dl></section>`;
  ...
  const recordSection = `<section class="receipt-section">${receiptSectionHeading('Record details', page)}<dl class="facts"><dt>payment</dt><dd>${escapeHtml(receipt.payment.maintainer_payment)} · ${receipt.payment.merge_contingent ? 'merge-contingent' : 'not merge-contingent'}</dd><dt>redactions</dt><dd>${escapeHtml(redactionsLabel(receipt.redactions))}</dd><dt>Bundle contents digest</dt><dd><code>${escapeHtml(receipt.bundle_digest)}</code></dd><dt>Signed asset SHA-256</dt><dd><code>${escapeHtml(attestationEvidence?.releaseAssetSha256 ?? 'not recorded')}</code></dd><dt>Signed provenance recorded</dt><dd>${attestationEvidence === null ? 'not verified' : `verified ${escapeHtml(attestationEvidence.verifiedAt)}`}</dd></dl></section>`;
  const limitationsSection = `<section class="receipt-section limitations">${receiptSectionHeading('NOT INCLUDED', page)}<ul>${receipt.limitations.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></section>`;
  ...
  const technicalEvidence = receipt.version === 2
    ? `<details class="evidence-drawer technical-evidence"><summary><span>Technical evidence</span><small>Code, environment, exact commands, and outputs</small></summary><div class="drawer-body">${projectSection}${renderCodeLines(receipt, { page })}${environmentSection}${checksSection}${publicScopeInterpretation}${patch}${rawOutput}</div></details>`
    : `${projectSection}${renderCodeLines(receipt, { page })}${environmentSection}${checksSection}${publicScopeInterpretation}`;
  const provenanceEvidence = receipt.version === 2
    ? `<details class="evidence-drawer provenance-evidence"><summary><span>Evidence &amp; limitations</span><small>Bundle identity, attestation, and claims boundary</small></summary><div class="drawer-body">${recordSection}${limitationsSection}${correction}${signedSection}</div></details>`
    : `${recordSection}${limitationsSection}${correction}${signedSection}`;
  return `<article class="receipt ${receiptClass(receipt)}${receipt.version === 2 ? ' receipt--economic' : ''}${featured ? ' receipt--featured' : ''}"${articleId}>
  ...
  ${renderEconomicOverview(receipt, {page})}
  ${technicalEvidence}
  ${provenanceEvidence}
  ...
  ${receipt.version === 2 ? '' : `${patch}${rawOutput}`}
  ${pageLink}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .receipt--economic { width:min(100%,58rem); padding:clamp(1rem,3vw,2rem); transform:none; border-top:8px solid #0b6849; box-shadow:0 1.6rem 4rem #0007; }
    ...
    .receipt--economic .receipt-head { display:grid; grid-template-columns:minmax(19rem,1fr) minmax(23rem,1fr); gap:.55rem 2rem; align-items:end; padding:.7rem 0 1.2rem; border-bottom:1px solid #b9c5ba; }
    .receipt--economic .receipt-head .brand,.receipt--economic .receipt-head h1,.receipt--economic .receipt-head h2,.receipt--economic .receipt-head .class-stamp { grid-column:1; }
    .receipt--economic .receipt-meta { grid-column:2; grid-row:1 / span 4; margin:0; align-self:center; }
    .receipt--economic .receipt-meta dd { font-size:.78rem; }
    ...
    .section-kicker { margin:0 0 .35rem; color:#27604c; font-size:.67rem; font-weight:900; letter-spacing:.13em; text-transform:uppercase; }
    .economic-overview { padding:1.35rem 0 1.15rem; border-bottom:1px solid #b9c5ba; }
    .economic-overview h2,.economic-identity h2,.economic-unknowns h2 { margin-bottom:.8rem; font-size:clamp(1.1rem,2.5vw,1.45rem); letter-spacing:-.02em; }
    .glance-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:.65rem; }
    .glance-card { min-height:7.4rem; padding:.8rem; display:flex; flex-direction:column; border:1px solid #c9c7ba; background:#f5f1e7; }
    .glance-card span { color:#5c645e; font-size:.65rem; font-weight:800; letter-spacing:.09em; text-transform:uppercase; }
    .glance-card strong { margin:auto 0 .25rem; font-size:1.05rem; line-height:1.14; }
    .glance-card small { color:#5c645e; font-size:.68rem; }
    .glance-card--pass { color:#064a34; border-color:#9bbdad; background:#dff1e7; }
    .glance-card--cost { border-color:#d5b765; background:#fff6d9; }
    .economic-identity { padding:1.45rem 0 1.25rem; border-bottom:1px solid #b9c5ba; }
    .section-heading { display:grid; grid-template-columns:minmax(12rem,1fr) minmax(16rem,1.2fr); gap:1rem 2rem; align-items:end; }
    .section-heading > p { margin-bottom:.8rem; color:#4b5750; font-size:.82rem; }
    .identity-grid { margin:.35rem 0 0; display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); border-top:1px solid #c9c7ba; border-left:1px solid #c9c7ba; }
    .identity-grid > div { min-width:0; padding:.8rem; border-right:1px solid #c9c7ba; border-bottom:1px solid #c9c7ba; }
    .identity-grid dt { margin-bottom:.35rem; }
    .identity-grid dd { font-weight:800; }
    .identity-grid dd span { display:block; margin-top:.28rem; color:#56615a; font-size:.72rem; font-weight:400; }
    .effort-strip { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:1px; margin-top:.85rem; background:#aebbb2; border:1px solid #aebbb2; }
    .effort-strip div { padding:.7rem; background:#17352d; color:#eff6ef; }
    .effort-strip span,.effort-strip strong { display:block; }
    .effort-strip span { color:#b9d4c8; font-size:.65rem; letter-spacing:.08em; text-transform:uppercase; }
    .effort-strip strong { margin-top:.25rem; font-size:.95rem; }
    .economic-unknowns { display:grid; grid-template-columns:1.25fr .9fr; gap:1.2rem 2rem; padding:1.35rem; margin:1.15rem 0; border-left:5px solid #b88b19; background:#fff6d9; }
    .economic-unknowns p { margin-bottom:.45rem; font-size:.82rem; }
    .economic-unknowns h3 { margin:.1rem 0 .45rem; color:#6a4b05; }
    .economic-unknowns ul { margin:0; padding-left:1.1rem; font-size:.78rem; }
    .evidence-drawer { margin:.7rem 0 0; border:1px solid #b9c5ba; background:#f8f5ed; }
    .evidence-drawer > summary { padding:.9rem 1rem; display:flex; justify-content:space-between; gap:1rem; align-items:center; cursor:pointer; list-style:none; color:#17352d; font-weight:900; }
    ...
    .evidence-drawer > summary span { font-size:.88rem; letter-spacing:.03em; }
    .evidence-drawer > summary small { margin-left:auto; color:#66736c; font-size:.68rem; font-weight:400; }
    .drawer-body,.drawer-grid { padding:0 1rem 1rem; border-top:1px solid #d3d3c8; }
    ...
    .drawer-grid h3 { margin-bottom:.55rem; }
    ...
    .attempt-list li span,.cost-lines li span { color:#56615a; font-size:.72rem; }
    .receipt--economic .receipt-disclosure { margin-top:1.3rem; padding:1rem; border:0; background:#17352d; color:#f2f6f2; }
    .receipt--economic .receipt-disclosure p { color:#c5d4cc; }
    ...
    @media (max-width:48rem) { .receipt--economic .receipt-head { grid-template-columns:1fr; } .receipt--economic .receipt-meta { grid-column:1; grid-row:auto; } .glance-grid,.identity-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } .section-heading,.economic-unknowns { grid-template-columns:1fr; } .effort-strip { grid-template-columns:repeat(2,minmax(0,1fr)); } .drawer-grid { grid-template-columns:1fr; } }
    @media (max-width:34rem) { .receipt { margin-left:0; margin-right:0; padding:.85rem; } .receipt-meta { grid-template-columns:1fr; } .facts { grid-template-columns:1fr; gap:.15rem; } .hero-notes,.glance-grid,.identity-grid,.effort-strip { grid-template-columns:1fr; } .economic-unknowns { padding:1rem; } .evidence-drawer > summary { align-items:flex-start; } .evidence-drawer > summary small { display:none; } .page-actions { align-items:flex-start; flex-direction:column; } .page-action-buttons { justify-content:flex-start; } dd { margin-bottom:.55rem; } }
    @media print { @page { margin:8mm; } :root,html,body { color-scheme:light; background:#fff; color:#000; } body { font-size:9pt; } main,.receipt-page { width:auto; padding:0; margin:0; } .mast,.counts,.gallery,.claims,.site-footer,.page-nav,.verify-ledger,.receipt-open,.outcome-stub,.request-run { display:none !important; } .patch,.evidence-output { display:none !important; } .receipt { display:block; width:auto; max-width:100%; margin:0; box-shadow:none; transform:none; } .receipt:not(.receipt--economic) { width:72mm; } .facts,.receipt-meta { grid-template-columns:1fr; gap:.15rem; } .facts dd,.receipt-meta dd { margin-bottom:.45rem; } .receipt .button-link,.receipt button { display:none; } .proof-scope,.limitations,.verification,.receipt-disclosure,.qr-link,.glance-card,.identity-grid>div { break-inside:avoid; page-break-inside:avoid; } .qr-link { display:flex; } .evidence-drawer:not([open]) > :not(summary) { display:block; } a { color:inherit; text-decoration:none; } }
  </style>
</head>
<body>
${body}
