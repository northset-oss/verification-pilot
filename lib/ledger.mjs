import { mkdir, readFile, readdir, writeFile } from 'node:fs/promises';
import path from 'node:path';

import { validateMission } from './mission-validator.mjs';

const PROJECTION_FIELDS = [
  'mission_id',
  'variant',
  'claims_tier',
  'grade',
  'target_repo',
  'issue_or_task',
  'consent_artifact',
  'maintainer_outcome',
  'run_record_bundle_digest',
  'attestation_uri',
  'disclosure_label',
];

function compareMissionIds(left, right) {
  if (left.mission_id < right.mission_id) return -1;
  if (left.mission_id > right.mission_id) return 1;
  return 0;
}

function projectMission(receipt) {
  const entry = {};
  for (const field of PROJECTION_FIELDS) {
    if (field === 'maintainer_outcome') {
      entry.maintainer_outcome = {
        status: receipt.maintainer_outcome.status,
        link: receipt.maintainer_outcome.link,
      };
    } else if (Object.hasOwn(receipt, field)) {
      entry[field] = receipt[field];
    }
  }
  entry.attested = typeof receipt.attestation_uri === 'string';
  entry.publication = null;
  return entry;
}

const PUBLICATION_STATES = new Set(['prepared', 'open', 'closed_unmerged', 'merged']);
const REVIEW_DECISIONS = new Set(['approved', 'changes_requested', 'review_required']);

export function validatePublication(value, missionId) {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new TypeError('publication must be an object');
  }
  if (value.schema_version !== 1 || value.mission_id !== missionId) {
    throw new TypeError('publication schema_version or mission_id is invalid');
  }
  if (!PUBLICATION_STATES.has(value.state)) throw new TypeError(`publication state is invalid: ${value.state}`);
  if (value.review_decision !== null && !REVIEW_DECISIONS.has(value.review_decision)) {
    throw new TypeError(`publication review_decision is invalid: ${value.review_decision}`);
  }
  for (const field of ['pr_url', 'pr_head_oid', 'decision_url', 'opened_at', 'closed_at', 'updated_at',
    'correction_note', 'attestation_uri', 'bundle_digest', 'release_asset_sha256', 'ci_state',
    'merge_commit_oid', 'base_branch']) {
    if (value[field] !== undefined && value[field] !== null && typeof value[field] !== 'string') {
      throw new TypeError(`publication ${field} must be a string or null`);
    }
  }
  if (value.pr_url !== null && value.pr_url !== undefined && !/^https:\/\/github\.com\//.test(value.pr_url)) {
    throw new TypeError('publication pr_url must be a GitHub URL or null');
  }
  if (value.pr_head_oid !== null && value.pr_head_oid !== undefined && !/^[0-9a-f]{40}$/i.test(value.pr_head_oid)) {
    throw new TypeError('publication pr_head_oid must be a full commit OID or null');
  }
  if (value.merge_commit_oid !== null && value.merge_commit_oid !== undefined && !/^[0-9a-f]{40}$/i.test(value.merge_commit_oid)) {
    throw new TypeError('publication merge_commit_oid must be a full commit OID or null');
  }
  if (value.pr_number !== null && value.pr_number !== undefined && (!Number.isInteger(value.pr_number) || value.pr_number < 1)) {
    throw new TypeError('publication pr_number must be a positive integer or null');
  }
  if (value.head_drift !== null && value.head_drift !== undefined && typeof value.head_drift !== 'boolean') {
    throw new TypeError('publication head_drift must be boolean or null');
  }
  return value;
}

async function publicationFor(missionFile, missionId) {
  const file = path.join(path.dirname(missionFile), 'publication.json');
  try {
    return validatePublication(JSON.parse(await readFile(file, 'utf8')), missionId);
  } catch (error) {
    if (error.code === 'ENOENT') return null;
    throw error;
  }
}

function formatValidationErrors(errors) {
  return errors
    .map((error) => `${error.ruleId} ${error.path}: ${error.message}`)
    .join('; ')
    .replaceAll(/\s+/g, ' ');
}

async function writeOutput(file, contents) {
  await mkdir(path.dirname(file), { recursive: true });
  await writeFile(file, contents, 'utf8');
}

/**
 * Build and write a deterministic public ledger index.
 *
 * @param {{missionsDir: string, out: string, now?: string|null, onWarning?: (message: string) => void}} options
 * @returns {Promise<{included: number, skipped: number, index: object}>}
 */
export async function buildLedger({ missionsDir, out, now = null, onWarning = () => {} }) {
  const directoryEntries = await readdir(missionsDir, { withFileTypes: true });
  const missionFiles = directoryEntries
    .filter((entry) => entry.isDirectory())
    .map((entry) => path.join(missionsDir, entry.name, 'mission.json'))
    .sort();

  const missions = [];
  let skipped = 0;

  for (const missionFile of missionFiles) {
    let receipt;
    try {
      receipt = JSON.parse(await readFile(missionFile, 'utf8'));
    } catch (error) {
      if (error.code === 'ENOENT') continue;
      skipped += 1;
      onWarning(`skipping ${missionFile}: ${error.message}`);
      continue;
    }

    const validation = validateMission(receipt);
    if (!validation.valid) {
      skipped += 1;
      onWarning(`skipping ${missionFile}: ${formatValidationErrors(validation.errors)}`);
      continue;
    }

    const projected = projectMission(receipt);
    try {
      projected.publication = await publicationFor(missionFile, receipt.mission_id);
    } catch (error) {
      skipped += 1;
      onWarning(`ignoring publication for ${missionFile}: ${error.message}`);
    }
    if (projected.publication !== null) {
      const publication = projected.publication;
      projected.maintainer_outcome = {status: publicationOutcome(publication), link: publication.decision_url ?? publication.pr_url};
      if (publication.bundle_digest) projected.run_record_bundle_digest = publication.bundle_digest;
      if (publication.attestation_uri) projected.attestation_uri = publication.attestation_uri;
      projected.attested = typeof projected.attestation_uri === 'string';
    }
    missions.push(projected);
  }

  missions.sort(compareMissionIds);
  const index = {
    version: '0',
    generated_at: now,
    missions,
  };
  await writeOutput(out, `${JSON.stringify(index, null, 2)}\n`);

  return { included: missions.length, skipped, index };
}

export function publicationOutcome(publication) {
  if (publication.state === 'open' && publication.review_decision === 'changes_requested') return 'changes_requested';
  return publication.state;
}

function serializeForInlineScript(value) {
  return JSON.stringify(value, null, 2)
    .replaceAll('&', '\\u0026')
    .replaceAll('<', '\\u003c')
    .replaceAll('>', '\\u003e')
    .replaceAll(' ', '\\u2028')
    .replaceAll(' ', '\\u2029');
}

function renderHtml(index) {
  const serializedIndex = serializeForInlineScript(index);
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Northset OSS Run Records</title>
  <style>
    :root {
      color-scheme: light dark;
      --bg: #f5f7f8;
      --panel: #ffffff;
      --ink: #111a1f;
      --muted: #566a74;
      --rule: #dbe3e7;
      --code-bg: #eef2f4;
      --accent: #0c7b54;
      --accent-weak: #e2f0ea;
      --link: #0a6699;
      --ok: #0c7b54;
      --ok-bg: #e2f0ea;
      --pending: #8f6410;
      --pending-bg: #f5ecd7;
      --sans: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
      --mono: ui-monospace, SFMono-Regular, Menlo, Consolas, "Liberation Mono", monospace;
    }

    * { box-sizing: border-box; }

    html, body { max-width: 100%; overflow-x: hidden; }

    body {
      margin: 0;
      background: var(--bg);
      color: var(--ink);
      font-family: var(--sans);
      line-height: 1.55;
      -webkit-font-smoothing: antialiased;
    }

    main {
      width: min(74rem, 100%);
      margin: 0 auto;
      padding: 2.5rem 1.15rem 4rem;
    }

    .mast {
      display: flex;
      flex-direction: column;
      gap: 0.4rem;
      margin: 0 0 1.9rem;
    }

    .logo { margin: 0; line-height: 0; }

    .logo-art {
      height: clamp(2.4rem, 6vw, 3.05rem);
      width: auto;
      display: block;
    }

    .logo-dark { display: none; }

    .visually-hidden {
      position: absolute;
      width: 1px;
      height: 1px;
      margin: -1px;
      padding: 0;
      border: 0;
      overflow: hidden;
      clip: rect(0 0 0 0);
      white-space: nowrap;
    }

    .domain {
      margin: 0;
      font-family: var(--mono);
      font-size: clamp(1rem, 2.6vw, 1.2rem);
      letter-spacing: 0.01em;
      color: var(--accent);
      font-weight: 600;
    }

    .domain .sep { color: var(--rule); padding: 0 0.15rem; }
    .domain .descriptor { color: var(--muted); font-weight: 500; }
    .domain a {
      color: inherit;
      text-decoration: underline;
      text-decoration-color: color-mix(in srgb, currentColor 45%, transparent);
      text-underline-offset: 3px;
    }
    .domain a:hover { text-decoration-color: currentColor; }

    .boundary {
      margin: 0 0 0.65rem;
      max-width: 48rem;
      color: var(--muted);
      font-size: 0.84rem;
    }

    .verify {
      background: var(--panel);
      border: 1px solid var(--rule);
      border-left: 3px solid var(--accent);
      border-radius: 0.6rem;
      padding: 1.1rem 1.25rem;
      margin: 0 0 1.5rem;
    }

    .verify h2 {
      margin: 0 0 0.4rem;
      font-size: 1.02rem;
    }

    .verify p {
      margin: 0 0 0.75rem;
      color: var(--muted);
      max-width: 46rem;
      font-size: 0.94rem;
    }

    .cmd {
      margin: 0;
      overflow-x: auto;
      background: var(--code-bg);
      border-radius: 0.4rem;
      padding: 0.7rem 0.85rem;
    }

    .cmd code {
      font-family: var(--mono);
      font-size: 0.82rem;
      color: var(--ink);
      white-space: pre;
    }

    .verify .caveat {
      margin: 0.7rem 0 0;
      font-size: 0.86rem;
    }

    .summary {
      display: flex;
      flex-wrap: wrap;
      gap: 0.5rem;
      margin: 0 0 1.75rem;
      padding: 0;
      list-style: none;
    }

    .chip {
      font-family: var(--mono);
      font-size: 0.78rem;
      color: var(--ink);
      background: var(--panel);
      border: 1px solid var(--rule);
      border-radius: 999px;
      padding: 0.28rem 0.7rem;
    }

    .chip b { color: var(--accent); font-weight: 700; }

    .legend {
      margin: 0 0 2rem;
      border-top: 1px solid var(--rule);
      padding-top: 1.1rem;
    }

    .legend h2 {
      margin: 0 0 0.75rem;
      font-family: var(--mono);
      font-size: 0.74rem;
      letter-spacing: 0.12em;
      text-transform: uppercase;
      color: var(--muted);
      font-weight: 600;
    }

    .legend dl {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(15.5rem, 1fr));
      gap: 0.7rem 1.5rem;
      margin: 0;
    }

    .legend dt {
      font-weight: 650;
      font-size: 0.9rem;
    }

    .legend dd {
      margin: 0.1rem 0 0;
      color: var(--muted);
      font-size: 0.88rem;
    }

    .table-wrap {
      max-width: 100%;
      overflow-x: auto;
      border: 1px solid var(--rule);
      border-radius: 0.6rem;
      background: var(--panel);
    }

    table {
      width: 100%;
      min-width: 46rem;
      border-collapse: collapse;
    }

    th, td {
      padding: 0.8rem 0.9rem;
      border-bottom: 1px solid var(--rule);
      text-align: left;
      vertical-align: top;
    }

    thead th {
      font-family: var(--mono);
      font-size: 0.68rem;
      letter-spacing: 0.09em;
      text-transform: uppercase;
      color: var(--muted);
      font-weight: 600;
      background: transparent;
    }

    tbody tr:last-child td { border-bottom: 0; }

    tbody tr:hover { background: color-mix(in srgb, var(--accent-weak) 45%, transparent); }

    .mid { font-family: var(--mono); font-weight: 650; font-size: 0.9rem; }

    a { color: var(--link); overflow-wrap: anywhere; }

    .repo { font-weight: 600; }

    .sub {
      display: inline-block;
      margin-top: 0.2rem;
      font-size: 0.82rem;
    }

    .disclosure {
      display: block;
      margin-top: 0.3rem;
      color: var(--muted);
      font-size: 0.8rem;
      max-width: 22rem;
    }

    .pill {
      display: inline-block;
      font-family: var(--mono);
      font-size: 0.75rem;
      padding: 0.14rem 0.55rem;
      border-radius: 999px;
      border: 1px solid transparent;
    }

    .pill-pending { color: var(--pending); background: var(--pending-bg); }
    .pill-ok { color: var(--ok); background: var(--ok-bg); }
    .pill-muted { color: var(--muted); background: var(--code-bg); }

    .badge {
      display: inline-flex;
      align-items: center;
      gap: 0.3rem;
      font-family: var(--mono);
      font-size: 0.78rem;
      font-weight: 600;
      text-decoration: none;
    }

    .badge.ok { color: var(--ok); font-weight: 650; }

    td code {
      display: inline-block;
      margin-top: 0.32rem;
      font-family: var(--mono);
      font-size: 0.74rem;
      color: var(--muted);
      background: var(--code-bg);
      border-radius: 0.25rem;
      padding: 0.1rem 0.32rem;
      white-space: nowrap;
    }

    footer {
      margin-top: 1.75rem;
      padding-top: 1.1rem;
      border-top: 1px solid var(--rule);
      color: var(--muted);
      font-size: 0.88rem;
    }

    footer a { color: var(--link); }

    @media (prefers-color-scheme: dark) {
      :root {
        --bg: #0d1316;
        --panel: #131c20;
        --ink: #e8eef0;
        --muted: #93a2aa;
        --rule: #253137;
        --code-bg: #0b1215;
        --accent: #34c08a;
        --accent-weak: #12241d;
        --link: #5cc3e6;
        --ok: #34c08a;
        --ok-bg: #12241d;
        --pending: #d2a45e;
        --pending-bg: #241c10;
      }

      .logo-light { display: none; }
      .logo-dark { display: block; }
    }
  </style>
</head>
<body>
  <main>
    <header class="mast">
      <h1 class="logo">
        <svg class="logo-art logo-light" role="img" aria-hidden="true" viewBox="0 18.35 475 96">
  <g transform="translate(-11.52,10.89) scale(1.075)"><g transform="translate(5.24,-0.04)"><rect x="18.5" y="26.0" width="11.0" height="56.0" fill="#162533"/><polygon points="26.6,26.0 29.4,26.0 56.79,62.82 74.03,20.0 85.97,20.0 59.4,83.0 19.64,31.36" fill="#162533"/></g></g><g transform="translate(110.18,99.0)"><path d="M5.93 0V-60.2H25.71L39.9 -6.97H41.28V-60.2H51.51V0H31.82L17.63 -53.23H16.17V0Z M81.25 1.2Q74.89 1.2 69.85 -1.42Q64.82 -4.04 61.94 -8.9Q59.06 -13.76 59.06 -20.55V-21.93Q59.06 -28.72 61.94 -33.63Q64.82 -38.53 69.85 -41.11Q74.89 -43.69 81.25 -43.69Q87.61 -43.69 92.6 -41.11Q97.59 -38.53 100.47 -33.63Q103.35 -28.72 103.35 -21.93V-20.55Q103.35 -13.76 100.47 -8.9Q97.59 -4.04 92.6 -1.42Q87.61 1.2 81.25 1.2ZM81.25 -7.57Q86.67 -7.57 90.11 -11.05Q93.55 -14.53 93.55 -20.81V-21.67Q93.55 -27.95 90.11 -31.43Q86.67 -34.92 81.25 -34.92Q75.83 -34.92 72.39 -31.43Q68.95 -27.95 68.95 -21.67V-20.81Q68.95 -14.53 72.39 -11.05Q75.83 -7.57 81.25 -7.57Z M111.24 0V-42.48H120.96V-37.58H122.42Q123.37 -40.25 125.61 -41.45Q127.84 -42.66 130.94 -42.66H136.1V-33.88H130.77Q126.47 -33.88 123.8 -31.6Q121.13 -29.33 121.13 -24.6V0Z M158.44 0Q154.31 0 151.86 -2.45Q149.41 -4.9 149.41 -9.12V-34.31H138.31V-42.48H149.41V-56.16H159.3V-42.48H171.51V-34.31H159.3V-10.75Q159.3 -8.17 161.71 -8.17H170.22V0Z M178.97 0V-60.2H188.86V-36.89H190.32Q191.01 -38.36 192.56 -39.82Q194.11 -41.28 196.64 -42.23Q199.18 -43.17 203.05 -43.17Q207.95 -43.17 211.74 -40.98Q215.52 -38.79 217.63 -34.83Q219.74 -30.87 219.74 -25.46V0H209.85V-24.68Q209.85 -29.84 207.31 -32.38Q204.77 -34.92 200.13 -34.92Q194.88 -34.92 191.87 -31.43Q188.86 -27.95 188.86 -21.5V0Z M246.81 1.2Q238.55 1.2 233.22 -2.41Q227.89 -6.02 226.77 -12.99L235.88 -15.31Q236.49 -12.04 238.03 -10.15Q239.58 -8.26 241.86 -7.48Q244.14 -6.71 246.81 -6.71Q250.85 -6.71 252.87 -8.17Q254.89 -9.63 254.89 -11.87Q254.89 -14.19 252.95 -15.31Q251.02 -16.43 247.06 -17.2L244.4 -17.63Q240.01 -18.49 236.4 -19.99Q232.79 -21.5 230.59 -24.12Q228.4 -26.75 228.4 -30.79Q228.4 -36.98 233 -40.33Q237.6 -43.69 245.09 -43.69Q252.22 -43.69 256.87 -40.51Q261.51 -37.32 262.89 -31.99L253.77 -29.24Q253.08 -32.85 250.76 -34.36Q248.44 -35.86 245.09 -35.86Q241.65 -35.86 239.8 -34.66Q237.95 -33.45 237.95 -31.22Q237.95 -28.98 239.84 -27.86Q241.73 -26.75 244.91 -26.23L247.58 -25.71Q252.31 -24.85 256.14 -23.48Q259.96 -22.1 262.2 -19.52Q264.44 -16.94 264.44 -12.56Q264.44 -5.93 259.66 -2.36Q254.89 1.2 246.81 1.2Z M291.25 1.2Q284.88 1.2 280.02 -1.5Q275.17 -4.21 272.46 -9.16Q269.75 -14.1 269.75 -20.73V-21.76Q269.75 -28.47 272.41 -33.37Q275.08 -38.27 279.9 -40.98Q284.71 -43.69 290.99 -43.69Q297.18 -43.69 301.83 -40.98Q306.47 -38.27 309.05 -33.37Q311.63 -28.47 311.63 -21.93V-18.4H279.72Q279.9 -13.42 283.25 -10.41Q286.6 -7.4 291.51 -7.4Q296.32 -7.4 298.64 -9.5Q300.97 -11.61 302.17 -14.28L310.34 -10.06Q309.14 -7.74 306.86 -5.12Q304.58 -2.49 300.79 -0.64Q297.01 1.2 291.25 1.2ZM279.81 -25.89H301.57Q301.22 -30.1 298.34 -32.59Q295.46 -35.09 290.9 -35.09Q286.17 -35.09 283.34 -32.59Q280.5 -30.1 279.81 -25.89Z M333.88 0Q329.76 0 327.3 -2.45Q324.85 -4.9 324.85 -9.12V-34.31H313.76V-42.48H324.85V-56.16H334.74V-42.48H346.96V-34.31H334.74V-10.75Q334.74 -8.17 337.15 -8.17H345.67V0Z" fill="#162533"/></g>
</svg>
        <svg class="logo-art logo-dark" role="img" aria-hidden="true" viewBox="0 18.35 475 96">
  <g transform="translate(-11.52,10.89) scale(1.075)"><g transform="translate(5.24,-0.04)"><rect x="18.5" y="26.0" width="11.0" height="56.0" fill="#FFFDF8"/><polygon points="26.6,26.0 29.4,26.0 56.79,62.82 74.03,20.0 85.97,20.0 59.4,83.0 19.64,31.36" fill="#FFFDF8"/></g></g><g transform="translate(110.18,99.0)"><path d="M5.93 0V-60.2H25.71L39.9 -6.97H41.28V-60.2H51.51V0H31.82L17.63 -53.23H16.17V0Z M81.25 1.2Q74.89 1.2 69.85 -1.42Q64.82 -4.04 61.94 -8.9Q59.06 -13.76 59.06 -20.55V-21.93Q59.06 -28.72 61.94 -33.63Q64.82 -38.53 69.85 -41.11Q74.89 -43.69 81.25 -43.69Q87.61 -43.69 92.6 -41.11Q97.59 -38.53 100.47 -33.63Q103.35 -28.72 103.35 -21.93V-20.55Q103.35 -13.76 100.47 -8.9Q97.59 -4.04 92.6 -1.42Q87.61 1.2 81.25 1.2ZM81.25 -7.57Q86.67 -7.57 90.11 -11.05Q93.55 -14.53 93.55 -20.81V-21.67Q93.55 -27.95 90.11 -31.43Q86.67 -34.92 81.25 -34.92Q75.83 -34.92 72.39 -31.43Q68.95 -27.95 68.95 -21.67V-20.81Q68.95 -14.53 72.39 -11.05Q75.83 -7.57 81.25 -7.57Z M111.24 0V-42.48H120.96V-37.58H122.42Q123.37 -40.25 125.61 -41.45Q127.84 -42.66 130.94 -42.66H136.1V-33.88H130.77Q126.47 -33.88 123.8 -31.6Q121.13 -29.33 121.13 -24.6V0Z M158.44 0Q154.31 0 151.86 -2.45Q149.41 -4.9 149.41 -9.12V-34.31H138.31V-42.48H149.41V-56.16H159.3V-42.48H171.51V-34.31H159.3V-10.75Q159.3 -8.17 161.71 -8.17H170.22V0Z M178.97 0V-60.2H188.86V-36.89H190.32Q191.01 -38.36 192.56 -39.82Q194.11 -41.28 196.64 -42.23Q199.18 -43.17 203.05 -43.17Q207.95 -43.17 211.74 -40.98Q215.52 -38.79 217.63 -34.83Q219.74 -30.87 219.74 -25.46V0H209.85V-24.68Q209.85 -29.84 207.31 -32.38Q204.77 -34.92 200.13 -34.92Q194.88 -34.92 191.87 -31.43Q188.86 -27.95 188.86 -21.5V0Z M246.81 1.2Q238.55 1.2 233.22 -2.41Q227.89 -6.02 226.77 -12.99L235.88 -15.31Q236.49 -12.04 238.03 -10.15Q239.58 -8.26 241.86 -7.48Q244.14 -6.71 246.81 -6.71Q250.85 -6.71 252.87 -8.17Q254.89 -9.63 254.89 -11.87Q254.89 -14.19 252.95 -15.31Q251.02 -16.43 247.06 -17.2L244.4 -17.63Q240.01 -18.49 236.4 -19.99Q232.79 -21.5 230.59 -24.12Q228.4 -26.75 228.4 -30.79Q228.4 -36.98 233 -40.33Q237.6 -43.69 245.09 -43.69Q252.22 -43.69 256.87 -40.51Q261.51 -37.32 262.89 -31.99L253.77 -29.24Q253.08 -32.85 250.76 -34.36Q248.44 -35.86 245.09 -35.86Q241.65 -35.86 239.8 -34.66Q237.95 -33.45 237.95 -31.22Q237.95 -28.98 239.84 -27.86Q241.73 -26.75 244.91 -26.23L247.58 -25.71Q252.31 -24.85 256.14 -23.48Q259.96 -22.1 262.2 -19.52Q264.44 -16.94 264.44 -12.56Q264.44 -5.93 259.66 -2.36Q254.89 1.2 246.81 1.2Z M291.25 1.2Q284.88 1.2 280.02 -1.5Q275.17 -4.21 272.46 -9.16Q269.75 -14.1 269.75 -20.73V-21.76Q269.75 -28.47 272.41 -33.37Q275.08 -38.27 279.9 -40.98Q284.71 -43.69 290.99 -43.69Q297.18 -43.69 301.83 -40.98Q306.47 -38.27 309.05 -33.37Q311.63 -28.47 311.63 -21.93V-18.4H279.72Q279.9 -13.42 283.25 -10.41Q286.6 -7.4 291.51 -7.4Q296.32 -7.4 298.64 -9.5Q300.97 -11.61 302.17 -14.28L310.34 -10.06Q309.14 -7.74 306.86 -5.12Q304.58 -2.49 300.79 -0.64Q297.01 1.2 291.25 1.2ZM279.81 -25.89H301.57Q301.22 -30.1 298.34 -32.59Q295.46 -35.09 290.9 -35.09Q286.17 -35.09 283.34 -32.59Q280.5 -30.1 279.81 -25.89Z M333.88 0Q329.76 0 327.3 -2.45Q324.85 -4.9 324.85 -9.12V-34.31H313.76V-42.48H324.85V-56.16H334.74V-42.48H346.96V-34.31H334.74V-10.75Q334.74 -8.17 337.15 -8.17H345.67V0Z" fill="#FFFDF8"/></g>
</svg>
        <span class="visually-hidden">Northset</span>
      </h1>
      <p class="domain"><a href="https://northset.ai">northset.ai</a> <span class="sep">&middot;</span> <span class="descriptor">open-source run records</span></p>
    </header>

    <section class="verify" aria-label="Verify a bundle yourself">
      <h2>Verify any bundle yourself</h2>
      <pre class="cmd"><code id="verify-command"></code></pre>
      <p class="caveat">Confirms the bundle&rsquo;s origin &mdash; that our signing workflow produced this exact file. The run itself is self-operated and self-reported; the signer doesn&rsquo;t witness it, and it isn&rsquo;t maintainer verification.</p>
    </section>

    <ul class="summary" id="summary"></ul>

    <section class="legend" aria-label="How to read this ledger">
      <h2>How to read this</h2>
      <dl>
        <div>
          <dt>Contribution</dt>
          <dd>Northset wrote the fix and ran the listed checks itself. The record is our own work &mdash; not maintainer verification.</dd>
        </div>
        <div>
          <dt>Own-repo rehearsal</dt>
          <dd>A run on Northset&rsquo;s own repository, used to exercise the pipeline. Not external validation.</dd>
        </div>
        <div>
          <dt>Attested</dt>
          <dd>GitHub&rsquo;s workflow signed the record bundle (SLSA provenance via Sigstore, logged in the public Rekor transparency log). The signature proves where the file came from, not that the code is good.</dd>
        </div>
        <div>
          <dt>Outcome</dt>
          <dd>The maintainer&rsquo;s decision on the contribution&rsquo;s pull request &mdash; the only real signal here, which Northset does not control. Rehearsals have none.</dd>
        </div>
      </dl>
    </section>

    <p class="boundary">A run record is evidence of declared execution metadata and artifacts. It is not proof of code quality, security, maintainer approval, or production readiness.</p>
    <div class="table-wrap">
      <table>
        <thead>
          <tr>
            <th scope="col">Mission</th>
            <th scope="col">Source</th>
            <th scope="col">Type</th>
            <th scope="col">Outcome</th>
            <th scope="col">Signed record</th>
          </tr>
        </thead>
        <tbody id="ledger-rows"></tbody>
      </table>
    </div>
    <footer>Self-funded field-testing. The reported signal is the external maintainer decision, which Northset does not control. Source and policies: <a href="https://github.com/northset-oss/verification-pilot">github.com/northset-oss/verification-pilot</a>.</footer>
  </main>
  <script type="application/json" id="ledger-data">${serializedIndex}</script>
  <script>
    (() => {
      const VERIFY_COMMAND = 'gh attestation verify <bundle> --repo northset-oss/verification-pilot --signer-workflow northset-oss/verification-pilot/.github/workflows/attest-bundle.yml';
      const VARIANT_LABELS = {
        own_repo_rehearsal: 'Own-repo rehearsal',
        author_contribution: 'Contribution',
        V: 'Verification',
        W: 'Worker mission',
        F: 'Funder mission',
      };
      const OUTCOME_CLASS = {
        pending: 'pill-pending',
        merged: 'pill-ok',
        approved: 'pill-ok',
        rejected: 'pill-muted',
        silent: 'pill-muted',
        withdrawn: 'pill-muted',
        open: 'pill-pending',
        changes_requested: 'pill-muted',
        closed_unmerged: 'pill-muted',
      };

      const data = JSON.parse(document.getElementById('ledger-data').textContent);
      const rows = document.getElementById('ledger-rows');

      const command = document.getElementById('verify-command');
      command.textContent = VERIFY_COMMAND;

      function el(tag, className, text) {
        const node = document.createElement(tag);
        if (className) node.className = className;
        if (text != null) node.textContent = text;
        return node;
      }

      function link(href, label, className) {
        const anchor = document.createElement('a');
        anchor.setAttribute('href', href);
        anchor.textContent = label;
        if (className) anchor.className = className;
        return anchor;
      }

      function pathLabel(value) {
        try {
          return new URL(value).pathname.replace(/^\\/+/, '') || value;
        } catch (error) {
          return value;
        }
      }

      function issueLabel(value) {
        try {
          const segments = new URL(value).pathname.split('/').filter(Boolean);
          const last = segments[segments.length - 1];
          return /^[0-9]+$/.test(last) ? '#' + last : 'task';
        } catch (error) {
          return 'task';
        }
      }

      function shortDigest(value) {
        if (typeof value !== 'string') return null;
        const hex = value.replace(/^sha256:/, '');
        return 'sha256:' + hex.slice(0, 10) + '\\u2026';
      }

      const summary = document.getElementById('summary');
      const external = data.missions.filter((mission) => mission.variant !== 'own_repo_rehearsal');
      const total = data.missions.length;
      const attestedCount = data.missions.filter((mission) => mission.attested).length;
      const mergedCount = external
        .filter((mission) => mission.maintainer_outcome.status === 'merged').length;
      const awaitingCount = external
        .filter((mission) => mission.publication?.state === 'open' && mission.publication?.review_decision !== 'changes_requested').length;

      function chip(count, label) {
        const item = el('li', 'chip');
        item.append(el('b', null, String(count)), document.createTextNode(' ' + label));
        return item;
      }
      const chips = [
        chip(total, total === 1 ? 'run record' : 'run records'),
        chip(attestedCount, 'attested'),
      ];
      if (mergedCount > 0) chips.push(chip(mergedCount, 'merged'));
      if (awaitingCount > 0) {
        chips.push(chip(awaitingCount, awaitingCount === 1 ? 'PR awaiting review' : 'PRs awaiting review'));
      }
      summary.append(...chips);

      for (const mission of data.missions) {
        const row = document.createElement('tr');
        row.id = mission.mission_id;

        const idCell = document.createElement('td');
        idCell.append(el('span', 'mid', mission.mission_id));
        row.append(idCell);

        const sourceCell = document.createElement('td');
        sourceCell.append(link(mission.target_repo, pathLabel(mission.target_repo), 'repo'));
        if (mission.issue_or_task) {
          sourceCell.append(
            document.createElement('br'),
            link(mission.issue_or_task, issueLabel(mission.issue_or_task), 'sub'),
          );
        }
        if (mission.publication?.pr_url) {
          sourceCell.append(document.createTextNode(' · '), link(mission.publication.pr_url, 'PR', 'sub'));
        }
        row.append(sourceCell);

        const typeCell = document.createElement('td');
        typeCell.append(el('span', null, VARIANT_LABELS[mission.variant] || mission.variant));
        if (mission.disclosure_label) {
          typeCell.append(el('small', 'disclosure', mission.disclosure_label));
        }
        row.append(typeCell);

        const outcomeCell = document.createElement('td');
        if (mission.variant === 'own_repo_rehearsal') {
          outcomeCell.append(el('span', 'pill pill-muted', 'n/a'));
        } else {
          const status = mission.maintainer_outcome.status;
          const label = status === 'pending' ? 'not published' : status.replaceAll('_', ' ');
          const pill = el('span', 'pill ' + (OUTCOME_CLASS[status] || 'pill-muted'), label);
          if (mission.maintainer_outcome.link) {
            const wrap = link(mission.maintainer_outcome.link, '', null);
            wrap.textContent = '';
            wrap.append(pill);
            outcomeCell.append(wrap);
          } else {
            outcomeCell.append(pill);
          }
        }
        row.append(outcomeCell);

        const recordCell = document.createElement('td');
        if (mission.attested) {
          recordCell.append(link(mission.attestation_uri, 'Attested', 'badge ok'));
          const digest = shortDigest(mission.run_record_bundle_digest);
          if (digest) {
            const code = document.createElement('code');
            code.textContent = digest;
            recordCell.append(document.createElement('br'), code);
          }
        } else {
          recordCell.append(el('span', 'pill pill-muted', 'not yet'));
        }
        row.append(recordCell);

        rows.append(row);
      }
    })();
  </script>
</body>
</html>
`;
}

/**
 * Render an index as a self-contained static HTML document.
 *
 * The optional now value is accepted by deterministic build invocations. The index's
 * generated_at remains the timestamp embedded in the rendered ledger data.
 *
 * @param {{indexPath: string, out: string, now?: string|null}} options
 * @returns {Promise<{missions: number}>}
 */
export async function renderLedger({ indexPath, out, now = null }) {
  if (now !== null && typeof now !== 'string') {
    throw new TypeError('now must be a string or null');
  }

  const index = JSON.parse(await readFile(indexPath, 'utf8'));
  if (typeof index !== 'object' || index === null || !Array.isArray(index.missions)) {
    throw new TypeError('index must be an object with a missions array');
  }

  await writeOutput(out, renderHtml(index));
  return { missions: index.missions.length };
}
