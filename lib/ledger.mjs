...
function renderReceiptPage(receipt, generatedAt, repositorySlugs) {
  ...
  return renderDocument({
    ...
    body: `<main class="receipt-page"><header class="page-nav">${pageActions}</header>${renderVerifyFirst(receipt)}${renderReceipt(receipt, { page: true, generatedAt })}${repositoryLink}${renderCiAgreementLine(receipt)}${renderRequestRunCta(repository?.label ?? repoLabel(receipt.target_repo))}${renderDiscrepancyPledge()}<section class="claims receipt-page-claims" aria-labelledby="receipt-claims-boundary"><h2 id="receipt-claims-boundary">Claims boundary</h2><p>${claimsDescription} It does not prove code quality, security, full CI coverage, production readiness, or maintainer approval. ${attestationClaim}</p><p>Read the full <a href="https://github.com/northset-oss/verification-pilot/blob/main/policies/claims_boundary.md">Claims Boundary policy</a>.</p></section><footer class="site-footer receipt-page-footer">A proof-of-pass receipt records that the declared commands returned exit 0 on the named code in the named environment. Maintainer outcome is reported separately and remains fully outside Northset’s control. Source: <a href="https://github.com/northset-oss/verification-pilot">northset-oss/verification-pilot</a>.</footer></main>`,
  });
}
