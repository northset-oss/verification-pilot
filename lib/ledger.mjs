...
export function validatePublication(value, missionId) {
  ...
  if (!SHA256_DIGEST_PATTERN.test(value.release_asset_sha256)) {
    throw new TypeError('publication release_asset_sha256 must be sha256: followed by 64 lowercase hex characters');
  }
  attestationAsset(value.attestation_uri, missionId, 'publication attestation_uri');
  const tag = new URL(value.attestation_uri).pathname.split('/').filter(Boolean)[4];
...
function renderReceipt(receipt, { featured = false, page = false } = {}) {
  ...
  const verification = receipt.attestation_uri === null
    ...
    : `<p>${link(receipt.attestation_uri, 'Download signed bundle', 'button-link')}</p>`;
  const attestationScope = receipt.attestation_uri === null
    ? ''
  ...
  const verify = receipt.verify_command === null
    ? ''
    : `<div class="verify-command"><p><strong>Verify this receipt</strong></p><pre><code>${escapeHtml(receipt.verify_command)}</code></pre><button type="button" data-copy="${escapeHtml(receipt.verify_command)}">Copy verify command</button></div>`;
  ...
  <section class="receipt-section">${receiptSectionHeading('Record details', page)}<dl class="facts"><dt>payment</dt><dd>${escapeHtml(receipt.payment.maintainer_payment)} · ${receipt.payment.merge_contingent ? 'merge-contingent' : 'not merge-contingent'}</dd><dt>redactions</dt><dd>${escapeHtml(redactionsLabel(receipt.redactions))}</dd><dt>Bundle contents digest</dt><dd><code>${escapeHtml(receipt.bundle_digest)}</code></dd><dt>Signed asset SHA-256</dt><dd><code>${escapeHtml(receipt.release_asset_sha256 ?? 'not recorded')}</code></dd><dt>Signed provenance recorded</dt><dd>${receipt.attestation_verified_at === null ? 'not verified' : `verified ${escapeHtml(receipt.attestation_verified_at)}`}</dd></dl></section>
...
function renderReceiptPage(receipt, generatedAt) {
  return renderDocument({
    ...
    body: `<main class="receipt-page"><header class="page-nav"><div class="page-actions"><a href="../../index.html">← All Proof-of-Pass Receipts</a><span class="page-action-buttons"><a class="button-link" href="receipt.json" download>Download receipt.json</a><button type="button" data-print>Print / Save receipt</button></span></div><p>Canonical receipt · ${escapeHtml(receipt.canonical_url)}</p><p>Generated at ${escapeHtml(generatedAt)}</p></header>${renderReceipt(receipt, { page: true })}<section class="claims receipt-page-claims" aria-labelledby="receipt-claims-boundary"><h2 id="receipt-claims-boundary">Claims boundary</h2><p>This page reports scoped proof-of-pass receipt evidence. It does not prove code quality, security, full CI coverage, production readiness, or maintainer approval. An attestation confirms bundle provenance; it does not broaden the receipt's claim.</p><p>Read the full <a href="https://github.com/northset-oss/verification-pilot/blob/main/policies/claims_boundary.md">Claims Boundary policy</a>.</p></section><footer class="site-footer receipt-page-footer">A proof-of-pass receipt records that the declared commands returned exit 0 on the named code in the named environment. Maintainer outcome is reported separately and remains fully outside Northset’s control. Source: <a href="https://github.com/northset-oss/verification-pilot">northset-oss/verification-pilot</a>.</footer></main>`,
  });
}
...
function publicReceiptSummary(receipt, generatedAt) {
  return {
    ...
    bundle: {
      ...
      signed_asset_sha256: receipt.release_asset_sha256,
      attestation_uri: receipt.attestation_uri,
      attestation_verified_at: receipt.attestation_verified_at,
      provenance: 'Signed provenance recorded; the signer records artifact origin, not execution witnessing or maintainer approval.',
    },
