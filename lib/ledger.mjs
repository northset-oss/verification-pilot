...
const CORRECTION_FIELDS = new Set([
  ...
  'reason',
  'source_url',
  'prior_rendered_sha256',
  'prior_rendered_bytes',
  'replacement_rendered_sha256',
  'superseded_claims',
]);
...
export function validatePublication(value, missionId) {
  ...
  if (![1, 2].includes(value.schema_version) || value.mission_id !== missionId) {
    throw new TypeError('publication schema_version or mission_id is invalid');
  }
  if (value.schema_version === 1 && (Object.hasOwn(value, 'listing') || Object.hasOwn(value, 'correction'))) {
    throw new TypeError('publication schema_version 1 cannot contain listing or correction');
  }
  if (value.schema_version === 2) {
    if (!Object.hasOwn(value, 'listing') || !Object.hasOwn(value, 'correction')) {
      throw new TypeError('publication schema_version 2 requires listing and correction');
    }
    validateListing(value.listing, 'publication listing');
    const correction = requiredObject(value.correction, 'publication correction');
    for (const field of CORRECTION_FIELDS) {
      if (!Object.hasOwn(correction, field)) {
        throw new TypeError(`publication correction.${field} is required`);
      }
    }
    for (const field of Object.keys(correction)) {
      if (!CORRECTION_FIELDS.has(field)) {
        throw new TypeError(`publication correction.${field} is not allowed`);
      }
    }
    requiredString(correction.correction_id, 'publication correction.correction_id');
    validTime(correction.corrected_at, 'publication correction.corrected_at');
    requiredString(correction.reason, 'publication correction.reason');
    validUrl(correction.source_url, 'publication correction.source_url');
    if (
      !SHA256_DIGEST_PATTERN.test(correction.prior_rendered_sha256)
      || !SHA256_DIGEST_PATTERN.test(correction.replacement_rendered_sha256)
    ) {
      throw new TypeError('publication correction rendered hashes must be SHA-256 digests');
    }
    if (!Number.isInteger(correction.prior_rendered_bytes) || correction.prior_rendered_bytes < 1) {
      throw new TypeError('publication correction.prior_rendered_bytes must be a positive integer');
    }
    requiredArray(correction.superseded_claims, 'publication correction.superseded_claims')
      .forEach((claim, index) => requiredString(claim, `publication correction.superseded_claims[${index}]`));
    if (value.listing !== 'correction_only') {
      throw new TypeError('publication schema_version 2 is reserved for correction-only records');
    }
  }
  ...
  if (value.state === 'prepared') {
    ...
    const requiredExternalFields = ['pr_head_oid', 'base_branch', 'opened_at', 'updated_at', 'observed_at'];
    if (value.schema_version === 1) requiredExternalFields.push('ci_state');
...
async function consentScopesFor(missionFile, missionId) {
  const missionDirectory = path.dirname(missionFile);
  const current = await readTextIfPresent(path.join(missionDirectory, 'consent-scopes.json'));
  if (current !== null) {
    try {
      return validateConsentScopes(JSON.parse(current), missionId);
    } catch (error) {
      throw new TypeError(`consent-scopes.json: ${error.message}`);
    }
  }
  const legacy = await readTextIfPresent(path.join(missionDirectory, 'consent.json'));
  if (legacy === null) return absentConsentScopes(missionId);
  try {
    return consentScopesFromLegacy(JSON.parse(legacy), missionId);
  } catch (error) {
    throw new TypeError(`consent.json: ${error.message}`);
  }
}
...
export async function buildReceiptViewModel({ missionFile, mission: suppliedMission = null, publication: suppliedPublication = undefined }) {
  ...
  const missionDirectory = path.dirname(missionFile);
  const consentScopes = await consentScopesFor(missionFile, mission.mission_id);
  ...
  const legacySelfRun = mission.variant === 'author_contribution' && patchCommit !== null;
  const publicListing = publicListingFor({publication, consent: consentScopes});
  ...
  return {
    ...
    classification: legacySelfRun
      ? 'LEGACY_SELF_RUN_RECORD — PATCH BYTES BOUND; PATCH COMMIT NOT EXECUTION-BOUND'
      : receiptClassification(mission.variant, mission.disclosure_label),
    evidence_classification: legacySelfRun ? 'LEGACY_SELF_RUN_RECORD' : 'CURRENT_RECEIPT_RECORD',
    ...
    correction: publication?.correction ?? null,
...
function factoryProofRawUrl(receipt) {
  const sourceUrl = receipt.source?.raw_proof_url;
  ...
  if (
    ...
    || segments[1] !== 'verification-pilot'
    || segments[2] !== 'blob'
    || !OID_PATTERN.test(segments[3])
    || segments[4] !== 'receipts'
    || segments[5] !== receipt.mission_id
    || segments[6] !== receipt.code.recorded_patch_commit
    || segments[7] !== 'proof.json'
  ) {
    return null;
  }
  return `https://raw.githubusercontent.com/northset-oss/verification-pilot/${segments.slice(3).join('/')}`;
}

function formatFactoryProofVerifyCommand(receipt) {
  if (factoryProofAttestation(receipt) === null) return null;
  const rawProofUrl = factoryProofRawUrl(receipt);
  if (rawProofUrl === null) return null;
  const subject = `${receipt.mission_id}-${receipt.code.recorded_patch_commit}-proof.json`;
  return `curl --fail --silent --show-error --location --output ${subject} '${rawProofUrl}'\n` +
    `gh attestation verify ${subject} --repo northset-oss/verification-pilot --signer-workflow northset-oss/verification-pilot/.github/workflows/pages.yml`;
}
...
function renderV1Overview(receipt, attestationEvidence) {
  ...
  <div class="proof-hero">
    ...
    <dl class="proof-status-rail" data-receipt-version="${receipt.version}">
      <div><dt>Scope</dt><dd><strong>recorded execution only</strong><span>No mutable upstream status is part of this receipt.</span></dd></div>
...
function renderReceipt(receipt, { featured = false, page = false, generatedAt = null } = {}) {
  ...
  const verification = attestationEvidence !== null
    ...
    : proofAttestation !== null
      ? `<p>${link(proofAttestation, 'Open factory proof attestation', 'button-link')}</p>`
  ...
  const attestationScope = attestationEvidence !== null
    ...
    : proofAttestation !== null
      ? '<p class="attestation-scope">This attestation covers the exact immutable factory proof bytes. It does not attest a legacy signed bundle, witness the recorded run, or turn contributor evidence into maintainer verification.</p>'
      : '';
  const verifyCommand = attestationEvidence !== null
    ? formatVerifyCommand(attestationEvidence.attestationUri)
    : formatFactoryProofVerifyCommand(receipt);
  const verify = verifyCommand === null
    ? ''
    : `<div class="verify-command"><p><strong>${proofAttestation === null ? 'Verify this receipt' : 'Verify the factory proof'}</strong></p><pre><code>${escapeHtml(verifyCommand)}</code></pre><button type="button" data-copy="${escapeHtml(verifyCommand)}">Copy verify command</button></div>`;
  ...
  const rawProof = receipt.source === undefined
    ? ''
    : `<dt>Immutable raw proof</dt><dd>${link(receipt.source.raw_proof_url, 'inspect source proof')}</dd><dt>Raw proof SHA-256</dt><dd>${renderCopyableHash(receipt.source.proof_sha256, 'raw proof SHA-256')}</dd>`;
  const attestationRecord = proofAttestation !== null
    ? `<dt>Factory proof attestation</dt><dd>${link(proofAttestation, 'recorded for the exact proof bytes')}</dd>`
    : `<dt>Signed asset SHA-256</dt><dd>${isTechnicalReceipt(receipt) ? renderCopyableHash(signedAssetHash, 'signed asset SHA-256') : `<code>${escapeHtml(signedAssetHash)}</code>`}</dd><dt>Signed provenance recorded</dt><dd>${attestationEvidence === null ? 'not verified' : `verified ${renderHumanTimestamp(attestationEvidence.verifiedAt)}`}</dd>`;
  const recordSection = `<section class="receipt-section">${receiptSectionHeading('Record details', page)}<dl class="facts"><dt>payment</dt><dd>${escapeHtml(receipt.payment.maintainer_payment)} · ${receipt.payment.merge_contingent ? 'merge-contingent' : 'not merge-contingent'}</dd><dt>redactions</dt><dd>${escapeHtml(redactionsLabel(receipt.redactions))}</dd><dt>Bundle contents digest</dt><dd>${bundleDigest}</dd>${rawProof}${attestationRecord}</dl></section>`;
  ...
  const signedSection = `<section class="receipt-section verification">${receiptSectionHeading(proofAttestation === null ? 'Signed bundle' : 'Factory proof attestation', page)}${verification}${verify}${attestationScope}<a class="qr-link" href="${escapeHtml(qrLink)}" aria-label="Canonical receipt page for ${escapeHtml(receipt.mission_id)}">${qr.svg}<span>QR → receipt page</span></a></section>`;
...
function renderPreview(receipt, {
  ...
  return `<article class="receipt-preview ${receiptClass(receipt)}"${anchor} aria-labelledby="${escapeHtml(labelId)}">
...
function renderVerifyFirst(receipt) {
  ...
  const proofAttestation = factoryProofAttestation(receipt);
  const command = evidence !== null
    ? formatVerifyCommand(evidence.attestationUri)
    : formatFactoryProofVerifyCommand(receipt);
  ...
  const description = proofAttestation === null
    ? ''
    : `<p>${link(proofAttestation, 'Factory proof attestation')} for the exact immutable proof bytes.</p>`;
  return `<section class="verify-first" aria-labelledby="verify-first-title"><h2 id="verify-first-title">Check this receipt without trusting this site</h2>${description}<pre><code>${escapeHtml(command)}</code></pre><button type="button" data-copy="${escapeHtml(command)}">Copy verify command</button><p>Expected output includes <code>Verification succeeded!</code></p></section>`;
}
...
function ogImageUrl(name) {
  return new URL(`og/${name}.svg`, SITE_BASE_URL).href;
}
...
function renderLedgerHtml(index) {
  ...
  const previews = receipts.slice().sort(byRecency).map((receipt) => renderPreview(receipt)).join('');
  ...
  return renderDocument({
    ...
    body: `<main>
  <header class="mast">${renderNorthsetWordmark()}<h1>Proof-of-Pass Receipts</h1><p>This technical ledger publishes only records with explicit receipt-publication consent. A receipt records which declared commands returned exit 0 on named code in a named environment. It is not a code-quality, security, maintainer-approval, or merge recommendation.</p></header>
  <section class="gallery" aria-labelledby="receipt-gallery"><div class="gallery-head"><div><p class="eyebrow">CONSENTED TECHNICAL RECORDS</p><h2 id="receipt-gallery">Published receipts</h2></div></div><div class="preview-grid">${previews}</div></section>
  <section class="claims"><p class="eyebrow">CLAIMS BOUNDARY</p><h2>What a receipt says</h2><p>Only the exact commands, recorded exit status, named code, and recorded environment are in scope. Attestation records artifact provenance; it does not witness execution or imply endorsement.</p><p>Mutable pull-request, review, and CI state are not part of immutable receipt pages.</p></section>
  <footer class="site-footer"><span class="footer-generated">Ledger generated <time datetime="${escapeHtml(index.generated_at)}">${escapeHtml(generatedLabel)}</time>.</span> Source: <a href="https://github.com/northset-oss/verification-pilot">northset-oss/verification-pilot</a>. Machine-readable: <a href="ledger.json">ledger.json</a> · <a href="schema/ledger.schema.json">JSON Schemas</a>.</footer>
</main>`,
  });
}
...
function renderReceiptPage(receipt, generatedAt) {
  ...
  const attestationClaim = factoryProofAttestation(receipt) === null
    ? "An attestation confirms bundle provenance; it does not broaden the receipt's claim."
    : "The factory proof attestation is bound to the exact proof bytes. It does not attest a legacy signed bundle or broaden the receipt's claim.";
  return renderDocument({
    ...
    body: `<main class="receipt-page"><header class="page-nav">${pageActions}</header>${renderVerifyFirst(receipt)}${renderReceipt(receipt, { page: true, generatedAt })}<section class="claims receipt-page-claims" aria-labelledby="receipt-claims-boundary"><h2 id="receipt-claims-boundary">Claims boundary</h2><p>${claimsDescription} It does not prove code quality, security, full CI coverage, production readiness, maintainer approval, or a recommendation to merge. ${attestationClaim}</p><p>Read the full <a href="https://github.com/northset-oss/verification-pilot/blob/main/policies/claims_boundary.md">Claims Boundary policy</a>.</p></section><footer class="site-footer receipt-page-footer">This page is a consented technical record. Mutable upstream status is not part of the receipt. Source: <a href="https://github.com/northset-oss/verification-pilot">northset-oss/verification-pilot</a>.</footer></main>`,
  });
}

function renderCorrectionOnlyPage(receipt) {
  const [heading, ...paragraphs] = receipt.correction_note.split('\n');
  const body = paragraphs.map((paragraph) => `<p>${escapeHtml(paragraph)}</p>`).join('');
  const source = receipt.correction.source_url;
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <title>${escapeHtml(receipt.mission_id)} correction</title>
  <style>
    :root { color-scheme:light; --ink:#17201c; --muted:#56635d; --paper:#fffdf7; --rule:#cfc9bb; --link:#075f43; }
    * { box-sizing:border-box; }
    body { margin:0; background:#ece9e0; color:var(--ink); font:16px/1.55 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; }
    main { width:min(52rem,calc(100% - 2rem)); margin:clamp(1rem,5vw,4rem) auto; padding:clamp(1.2rem,4vw,3rem); background:var(--paper); border:1px solid var(--rule); }
    h1 { margin:.25rem 0 1.5rem; font:700 clamp(1.8rem,5vw,3.2rem)/1.08 system-ui,sans-serif; letter-spacing:-.04em; }
    h2 { margin-top:2rem; font:700 1.15rem/1.25 system-ui,sans-serif; }
    .eyebrow { color:var(--muted); font-size:.75rem; font-weight:700; letter-spacing:.1em; }
    dl { display:grid; grid-template-columns:minmax(10rem,max-content) minmax(0,1fr); gap:.45rem 1rem; }
    dt { color:var(--muted); font-size:.75rem; text-transform:uppercase; }
    dd { margin:0; min-width:0; overflow-wrap:anywhere; }
    code { font:inherit; overflow-wrap:anywhere; white-space:pre-wrap; }
    pre { margin:.35rem 0 0; padding:.75rem; overflow:auto; border:1px solid var(--rule); background:#f4f0e6; white-space:pre-wrap; }
    a { color:var(--link); overflow-wrap:anywhere; }
    footer { margin-top:2rem; padding-top:1rem; border-top:1px solid var(--rule); }
    @media (max-width:36rem) { dl { grid-template-columns:1fr; } dd { margin-bottom:.6rem; } }
  </style>
</head>
<body>
<main>
  <article>
    <p class="eyebrow">CORRECTION-ONLY RECORD · UNLISTED</p>
    <h1>${escapeHtml(heading)}</h1>
    ${body}
    <section aria-labelledby="correction-scope"><h2 id="correction-scope">Immutable scope retained</h2><p>${escapeHtml(receipt.scope_note)}</p><dl><dt>Evidence classification</dt><dd><code>${escapeHtml(receipt.evidence_classification)}</code></dd><dt>Patch bytes SHA-256</dt><dd><code>${escapeHtml(receipt.code.patch_diff_hash)}</code></dd><dt>Bundle contents SHA-256</dt><dd><code>${escapeHtml(receipt.bundle_digest)}</code></dd><dt>Signed asset SHA-256</dt><dd><code>${escapeHtml(receipt.release_asset_sha256)}</code></dd><dt>Signed artifact</dt><dd>${link(receipt.attestation_uri, receipt.attestation_uri)}</dd><dt>Artifact verification command</dt><dd><pre><code>${escapeHtml(receipt.verify_command)}</code></pre></dd><dt>Prior rendered page SHA-256</dt><dd><code>${escapeHtml(receipt.correction.prior_rendered_sha256)}</code></dd><dt>Prior rendered page bytes</dt><dd><code>${escapeHtml(receipt.correction.prior_rendered_bytes)}</code></dd><dt>Correction source</dt><dd>${link(source, source)}</dd><dt>Corrected at</dt><dd><time datetime="${escapeHtml(receipt.correction.corrected_at)}">${escapeHtml(receipt.correction.corrected_at)}</time></dd></dl></section>
    <footer><strong>LEGACY_SELF_RUN_RECORD</strong><p>The recorded patch bytes are bound. The declared patch commit and later pull-request head are not execution-bound identities.</p></footer>
  </article>
</main>
</body>
</html>
...
function publicCorrectionSummary(receipt, generatedAt) {
  return {
    schema_version: 1,
    generated_at: generatedAt,
    receipt_id: receipt.mission_id,
    listing: 'correction_only',
    evidence_classification: receipt.evidence_classification,
    correction: {...receipt.correction},
    correction_text: receipt.correction_note,
    immutable_scope: {
      patch_diff_hash: receipt.code.patch_diff_hash,
      bundle_digest: receipt.bundle_digest,
      scope_note: receipt.scope_note,
    },
    artifact_verification: {
      attestation_uri: receipt.attestation_uri,
      release_asset_sha256: receipt.release_asset_sha256,
      verify_command: receipt.verify_command,
    },
  };
}
...
function publicReceiptSummary(receipt, generatedAt) {
  ...
  return {
    ...
    evidence_classification: receipt.evidence_classification,
    consent_scopes: receipt.consent_scopes,
    links: {
    ...
    ...(receipt.version === 3 ? {
      ...
      source: {
        receipt_class: receipt.source.receipt_class,
        proof_schema_version: receipt.source.proof_schema_version,
        proof_sha256: receipt.source.proof_sha256,
        raw_proof_url: receipt.source.raw_proof_url,
      },
    } : {}),
  };
}
...
export async function renderLedger({ indexPath, out, now = null }) {
  ...
  const listedMissions = index.missions.filter((mission) => mission.receipt.public_listing === 'listed');
  const correctionMissions = index.missions.filter((mission) => mission.receipt.public_listing === 'correction_only');
  const renderedMissions = [...listedMissions, ...correctionMissions];
  const publicIndex = {...index, missions: listedMissions};
  await mkdir(receiptsRoot, { recursive: true });
  await mkdir(ogRoot, { recursive: true });
  await rm(repositoriesRoot, {recursive: true, force: true});
  await writeOutput(out, renderLedgerHtml(publicIndex));
  await writeOutput(path.join(ogRoot, 'index.svg'), renderHomepageOgSvg(publicIndex));
  const publicReceipts = listedMissions.map((mission) => publicReceiptSummary(mission.receipt, index.generated_at));
  await writeOutput(
  ...
  for (const mission of renderedMissions) {
    ...
    if (receipt.public_listing === 'correction_only') {
      await writeOutput(
        path.join(receiptDirectory, 'index.html'),
        renderCorrectionOnlyPage(receipt, index.generated_at),
      );
      await writeOutput(
        path.join(receiptDirectory, 'receipt.json'),
        `${JSON.stringify(publicCorrectionSummary(receipt, index.generated_at), null, 2)}\n`,
      );
      await rm(path.join(ogRoot, `${receipt.mission_id}.svg`), {force: true});
      await rm(path.join(ogRoot, `${receipt.mission_id}.png`), {force: true});
      continue;
    }
    await writeOutput(
      path.join(receiptDirectory, 'index.html'),
      renderReceiptPage(receipt, index.generated_at),
    );
  ...
  for (const entry of await readdir(ogRoot, { withFileTypes: true })) {
    ...
    const missionId = ['.svg', '.png'].includes(extension) ? entry.name.slice(0, -extension.length) : '';
    if (GENERATED_RECEIPT_PATTERN.test(missionId) && !renderedIds.has(missionId)) {
      await rm(path.join(ogRoot, entry.name), { force: true });
    }
  }
  await validateRenderedPublication({siteRoot, index});
  return {
    missions: index.missions.length,
    listed: listedMissions.length,
    corrections: correctionMissions.length,
    pages: renderedMissions.length + 1,
  };
}
