...
function renderCodeLines(receipt, { page = false } = {}) {
  const lines = [
    ...
    receipt.code.patch_diff_hash === null ? '' : `<dt>patch diff SHA-256</dt><dd>${receipt.version === 1 ? renderCopyableHash(receipt.code.patch_diff_hash, 'patch diff SHA-256') : `<code>${escapeHtml(receipt.code.patch_diff_hash)}</code>`}<br><span>${escapeHtml(receipt.code.patch_diff_binding)}</span></dd>`,
  ].filter(Boolean).join('');
...
function renderV1Verdict(receipt) {
  if (receipt.version !== 1) return '';
  const titleId = `technical-result-${receipt.mission_id}`;
  return `<section class="v1-verdict" aria-labelledby="${escapeHtml(titleId)}">
  <p class="section-kicker">TECHNICAL RESULT</p>
  <h2 class="v1-verdict-result" id="${escapeHtml(titleId)}">${escapeHtml(receipt.result)}</h2>
  <p class="v1-execution-summary">${escapeHtml(receipt.execution_summary)}</p>
  <p class="v1-verdict-scope">Recorded on the named code in the named execution environment. Only the declared commands are in scope.</p>
</section>`;
}

function renderCryptographicDetail(receipt, attestationEvidence) {
  if (receipt.version !== 1) return '';
  const fields = [
  ...
  return `<details class="cryptographic-detail"><summary>Cryptographic detail</summary><p>Full recorded values. The compact receipt above shortens these for legibility only.</p><dl class="facts">${values}</dl></details>`;
}
...
function renderReceipt(receipt, { featured = false, page = false, generatedAt = null } = {}) {
  ...
  const runStartParts = receipt.started_at.split('T');
  const runFinishParts = receipt.finished_at.split('T');
  const runStartLabel = runStartParts.length === 2 ? `${runStartParts[0]} ${runStartParts[1]}` : receipt.started_at;
  const runFinishLabel = runFinishParts.length === 2 && runFinishParts[0] === runStartParts[0]
    ? runFinishParts[1]
    : receipt.finished_at.replace('T', ' ');
  ...
  const environmentDigest = receipt.environment.container_image_digest ?? 'not available in this legacy record';
  const environmentSection = `<section class="receipt-section">${receiptSectionHeading('Environment', page)}<dl class="facts"><dt>image reference</dt><dd>${escapeHtml(receipt.environment.container_image_ref)}</dd><dt>repository digest</dt><dd>${receipt.version === 1 ? renderCopyableHash(environmentDigest, 'container image digest') : `<code>${escapeHtml(environmentDigest)}</code>`}</dd>${receipt.environment.container_image_id === null ? '' : `<dt>immutable image ID</dt><dd>${receipt.version === 1 ? renderCopyableHash(receipt.environment.container_image_id, 'immutable image ID') : `<code>${escapeHtml(receipt.environment.container_image_id)}</code>`}</dd>`}${receipt.environment.container_os === null ? '' : `<dt>platform</dt><dd>${escapeHtml(receipt.environment.container_os)}/${escapeHtml(receipt.environment.container_architecture)}</dd>`}<dt>network</dt><dd>${escapeHtml(receipt.environment.network_policy)}</dd></dl></section>`;
  const checksSection = `<section class="receipt-section proof-scope">${receiptSectionHeading('Declared checks', page)}<p><strong>Execution summary</strong><br>${escapeHtml(receipt.execution_summary)}</p><ol class="commands">${commandLines}</ol>${setupDuration}${wallDuration}${receipt.version === 2 ? `<p class="total">${escapeHtml(receipt.result)}</p>` : ''}<p class="scope-note">${escapeHtml(scopeNote)}</p></section>`;
  const signedAssetHash = attestationEvidence?.releaseAssetSha256 ?? 'not recorded';
  const recordSection = `<section class="receipt-section">${receiptSectionHeading('Record details', page)}<dl class="facts"><dt>payment</dt><dd>${escapeHtml(receipt.payment.maintainer_payment)} · ${receipt.payment.merge_contingent ? 'merge-contingent' : 'not merge-contingent'}</dd><dt>redactions</dt><dd>${escapeHtml(redactionsLabel(receipt.redactions))}</dd><dt>Bundle contents digest</dt><dd>${receipt.version === 1 ? renderCopyableHash(receipt.bundle_digest, 'bundle contents digest') : `<code>${escapeHtml(receipt.bundle_digest)}</code>`}</dd><dt>Signed asset SHA-256</dt><dd>${receipt.version === 1 ? renderCopyableHash(signedAssetHash, 'signed asset SHA-256') : `<code>${escapeHtml(signedAssetHash)}</code>`}</dd><dt>Signed provenance recorded</dt><dd>${attestationEvidence === null ? 'not verified' : `verified ${renderHumanTimestamp(attestationEvidence.verifiedAt)}`}</dd></dl></section>`;
  const limitations = receipt.version === 1 && receipt.scope_note !== null
  ...
  const evidence = receipt.version === 2
    ? `<details class="evidence-drawer evidence-annex"><summary><span>Evidence annex</span><small>Economic · technical · provenance · limitations</small></summary><div class="annex-body">${renderEconomicEvidence(receipt)}${technicalChapter}${provenanceChapter}</div></details>`
    : `${projectSection}${renderCodeLines(receipt, { page })}${environmentSection}${checksSection}${publicScopeInterpretation}${recordSection}${renderCryptographicDetail(receipt, attestationEvidence)}${limitationsSection}${correction}${signedSection}`;
  const receiptHeader = receipt.version === 2
    ? `<header class="receipt-head">
    <span class="folio-watermark" aria-hidden="true">${escapeHtml(folioNumber)}</span>
    <div class="folio-title"><p class="brand">NORTHSET</p><${folioHeadingLevel}>Proof-of-Pass Receipt</${folioHeadingLevel}><p class="folio-receipt-id"><span>Receipt ID</span> <code>${escapeHtml(receipt.mission_id)}</code></p></div>
    <p class="folio-work"><span>ISSUE / WORK</span>${work}</p>
    <p class="class-stamp">${escapeHtml(receipt.classification)}</p>
    ${consent}
    <p class="run-interval" aria-label="Run interval from ${escapeHtml(receipt.started_at)} to ${escapeHtml(receipt.finished_at)}"><span>RUN</span><time datetime="${escapeHtml(receipt.started_at)}">${escapeHtml(runStartLabel)}</time><b aria-hidden="true">→</b><time datetime="${escapeHtml(receipt.finished_at)}">${escapeHtml(runFinishLabel)}</time></p>
  </header>`
    : `<header class="receipt-head">
    ...
    <p class="class-stamp">${escapeHtml(receipt.classification)}</p>
    ${consent}
    <dl class="receipt-meta"><div><dt>Receipt ID</dt><dd>${escapeHtml(receipt.mission_id)}</dd></div><div><dt>Recorded run</dt><dd>${renderRunInterval(receipt.started_at, receipt.finished_at, receipt.wall_duration_ms)}</dd></div></dl>
  </header>`;
  return `<article class="receipt ${receiptClass(receipt)}${receipt.version === 2 ? ' receipt--economic' : ''}${featured ? ' receipt--featured' : ''}"${articleId}>
  ${receiptHeader}
  ${renderV1Verdict(receipt)}
...
function renderPreview(receipt, { includeAnchor = true } = {}) {
  ...
  ${drift}
  ${relativeLink(receipt.canonical_path, 'Open receipt →', 'preview-link')}
</article>`.replaceAll(/^[ \t]+$/gm, '');
}
...
function renderReceiptPage(receipt, generatedAt) {
  const legacyPageMetadata = receipt.version === 2
    ? ''
    : `<p>Canonical receipt · ${escapeHtml(receipt.canonical_url)}</p><p>Receipt page generated <time datetime="${escapeHtml(generatedAt)}">${escapeHtml(formatHumanDate(generatedAt))}</time></p>`;
  const pageActions = receipt.version === 2
    ? '<div class="page-actions"><a href="../../index.html">← Receipts</a><span class="page-action-buttons"><a class="button-link" href="receipt.json" download aria-label="Download receipt.json">JSON</a><button type="button" data-print aria-label="Print / Save receipt">Print / Save</button></span></div>'
    : '<div class="page-actions"><a href="../../index.html">← All Proof-of-Pass Receipts</a><span class="page-action-buttons"><a class="button-link" href="receipt.json" download>Download receipt.json</a><button type="button" data-print>Print / Save receipt</button></span></div>';
  return renderDocument({
    title: `${receipt.mission_id} Proof-of-Pass Receipt`,
    body: `<main class="receipt-page"><header class="page-nav">${pageActions}${legacyPageMetadata}</header>${renderReceipt(receipt, { page: true, generatedAt })}${renderRequestRunCta()}<section class="claims receipt-page-claims" aria-labelledby="receipt-claims-boundary"><h2 id="receipt-claims-boundary">Claims boundary</h2><p>This page reports scoped proof-of-pass receipt evidence. It does not prove code quality, security, full CI coverage, production readiness, or maintainer approval. An attestation confirms bundle provenance; it does not broaden the receipt's claim.</p><p>Read the full <a href="https://github.com/northset-oss/verification-pilot/blob/main/policies/claims_boundary.md">Claims Boundary policy</a>.</p></section><footer class="site-footer receipt-page-footer">A proof-of-pass receipt records that the declared commands returned exit 0 on the named code in the named environment. Maintainer outcome is reported separately and remains fully outside Northset’s control. Source: <a href="https://github.com/northset-oss/verification-pilot">northset-oss/verification-pilot</a>.</footer></main>`,
  });
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .mast { max-width:68ch; margin-bottom:2rem; } .mast-cta { margin:1.2rem 0 .8rem; }
    ...
    .v1-verdict { margin:.9rem 0 0; padding:1rem .9rem; border-block:3px double #4e7666; background:#e2eee8; } .v1-verdict .section-kicker { margin-bottom:.45rem; } .v1-verdict-result { max-width:none; margin:0; color:#07583e; font-family:var(--sans); font-size:clamp(1.7rem,6vw,2.5rem); line-height:.98; letter-spacing:-.04em; text-transform:none; } .v1-execution-summary { margin:.7rem 0 .25rem; color:#183c30; font-family:var(--sans); font-weight:750; } .v1-verdict-scope { margin:0; color:#4a5b53; font-size:.76rem; }
    ...
    .receipt-page > .request-run { margin-top:1.5rem; padding:.8rem 0 0; border:0; border-top:1px solid var(--line); background:transparent; opacity:.68; font-size:.72rem; }
    .receipt-page > .request-run .eyebrow { margin-bottom:.25rem; color:var(--muted); font-size:.65rem; }
    .receipt-page > .request-run h2 { margin-bottom:.45rem; font-size:1rem; letter-spacing:0; }
    .receipt-page > .request-run .request-actions { margin:.65rem 0 .5rem; }
    ...
    @media print { @page { margin:8mm; } :root,html,body { color-scheme:light; background:#fff; color:#000; } body { font-size:9pt; } main,.receipt-page { width:auto; padding:0; margin:0; } .mast,.hero-stats,.gallery,.claims,.site-footer,.page-nav,.verify-ledger,.receipt-open,.outcome-stub,.request-run { display:none !important; } .patch,.evidence-output { display:none !important; } .receipt { display:block; width:auto; max-width:100%; margin:0; box-shadow:none; transform:none; } .receipt--economic { background:#fff; border:1px solid #777; } .receipt--economic::before,.receipt--economic::after { display:none; } .receipt:not(.receipt--economic) { width:72mm; } .facts,.receipt-meta { grid-template-columns:1fr; gap:.15rem; } .facts dd,.receipt-meta dd { margin-bottom:.45rem; } .receipt .button-link,.receipt button { display:none; } .v1-verdict,.proof-scope,.cryptographic-detail,.limitations,.verification,.receipt-disclosure,.qr-link,.proof-hero,.proofline-anatomy,.proofline-stage,.identity-flow>li,.receipt-cost-total,.receipt-provenance { break-inside:avoid; page-break-inside:avoid; } .anatomy-segment { animation:none; } .qr-link { display:flex; } .evidence-drawer:not([open]) > :not(summary),.annex-chapter:not([open]) > :not(summary),.cryptographic-detail:not([open]) > :not(summary) { display:block !important; } a { color:inherit; text-decoration:none; } }
  </style>
</head>
<body>
${body}
