...
const MERGED_INVITATION = 'Maintainers can request a separate, private run for a PR already in their queue at oss@northset.ai.';
...
export function renderDisclosureBlock({ missionId, receiptUrl, publicationState }) {
  validateMissionId(missionId);
...
export function upsertDisclosureBlock(body, { missionId, receiptUrl, publicationState }) {
  if (typeof body !== 'string') throw new TypeError('pull request body must be a string');
  const block = renderDisclosureBlock({ missionId, receiptUrl, publicationState });
...
async function auditRemote({ missionId, publication, policy, request, requireBodyLink = true }) {
  ...
  if (requireBodyLink) {
    const expectedBlock = renderDisclosureBlock({
      missionId,
      receiptUrl: canonicalUrl,
      publicationState: publication.state,
    });
    if (!pr.body.includes(expectedBlock)) {
      throw new Error(`${missionId} ${publication.state} PR body must contain the expected state-specific marked block`);
    }
    const invitationOccurrences = countOccurrences(pr.body, MERGED_INVITATION);
    const expectedInvitations = publication.state === 'merged' ? 1 : 0;
    if (invitationOccurrences !== expectedInvitations) {
      throw new Error(`${missionId} ${publication.state} PR body must contain ${expectedInvitations} merged-only invitation(s); found ${invitationOccurrences}`);
    }
  }
