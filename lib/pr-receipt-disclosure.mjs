...
const DISCLOSURE_PUBLICATION_ACTIONS = new Set(['opened', 'updated']);
...
export function renderDisclosureBlock({
  ...
  if (!DISCLOSURE_PUBLICATION_ACTIONS.has(publicationAction)) {
    throw new TypeError('publicationAction must be opened or updated');
  }
  ...
  return [
    ...
    '### Verification',
    '',
    `${renderCommand(command)} exited 0 on this exact head (\`${headOid.slice(0, 7)}\`) in a network-off container, before this PR ${publicationAction === 'updated' ? 'update was published' : 'was opened'}.`,
...
function exactPullRequest(publication) {
  ...
  return {
    ...
    commitsUrl: `https://api.github.com/repos/${match[1]}/${match[2]}/pulls/${match[3]}/commits?per_page=100`,
...
async function fetchPullRequestCommitOids({ request, url }) {
  const commits = [];
  const visited = new Set();
  const endpoint = new URL(url);
  let next = url;
  while (next !== null) {
    if (visited.has(next) || visited.size >= 20) throw new Error('GitHub PR commit pagination is invalid or excessive');
    const page = new URL(next);
    const allowedQuery = [...page.searchParams.keys()].every((key) => ['page', 'per_page'].includes(key));
    if (
      page.origin !== 'https://api.github.com'
      ...
      throw new Error('PR commit pagination must stay on the original GitHub API endpoint');
    }
    visited.add(next);
    const response = await requireResponse(request, next, {}, 'GitHub PR commits API');
    if (!Array.isArray(response.json)) throw new Error('GitHub PR commits API must return an array');
    for (const commit of response.json) {
      if (!isObject(commit) || typeof commit.sha !== 'string' || !OID_PATTERN.test(commit.sha)) {
        throw new Error('GitHub PR commits API returned an invalid commit');
      }
...
async function fetchBodyEdits({ request, owner, repository, prNumber }) {
  const edits = [];
  const visited = new Set();
  let cursor = null;
  let totalCount = null;
  for (let page = 0; page < 20; page += 1) {
    if (cursor !== null) {
      if (visited.has(cursor)) throw new Error('GitHub PR body edit pagination repeated a cursor');
      visited.add(cursor);
    }
    const response = await requireResponse(
      ...
      'https://api.github.com/graphql',
      {
        method: 'POST',
        body: {
          query: 'query($owner:String!,$name:String!,$number:Int!,$cursor:String){repository(owner:$owner,name:$name){pullRequest(number:$number){userContentEdits(first:100,after:$cursor){totalCount nodes{editedAt deletedAt diff} pageInfo{hasNextPage endCursor}}}}}',
          variables: { owner, name: repository, number: prNumber, cursor },
        },
      },
      'GitHub PR body edit history',
    );
    if (Array.isArray(response.json?.errors) && response.json.errors.length > 0) {
      throw new Error('GitHub PR body edit history GraphQL response contains errors');
    }
    const connection = response.json?.data?.repository?.pullRequest?.userContentEdits;
    if (
      !isObject(connection)
      ...
      throw new Error('GitHub PR body edit history returned an invalid response');
    }
    totalCount ??= connection.totalCount;
    if (connection.totalCount !== totalCount) {
      throw new Error('GitHub PR body edit history totalCount changed during pagination');
    }
    for (const edit of connection.nodes) {
      if (!isObject(edit) || edit.deletedAt !== null || typeof edit.diff !== 'string') {
        throw new Error('GitHub PR body edit history contains an unavailable revision');
      }
      edits.push({
        editedAt: isoTime(edit.editedAt, 'GitHub PR body edit editedAt'),
        body: edit.diff.replace(/\r\n?/g, '\n'),
      });
    }
    if (connection.pageInfo.hasNextPage !== true) {
      if (edits.length !== totalCount) {
        throw new Error('GitHub PR body edit history did not return every revision');
      }
      return edits;
    }
    if (typeof connection.pageInfo.endCursor !== 'string' || connection.pageInfo.endCursor === '') {
      throw new Error('GitHub PR body edit history pagination is invalid');
    }
    cursor = connection.pageInfo.endCursor;
  }
  throw new Error('GitHub PR body edit history pagination is excessive');
}
...
async function auditRemote({
  ...
  const { owner, repository, prNumber, apiUrl, commitsUrl, commentsUrl } = exactPullRequest(publication);
  ...
  if (
    expectedLiveHeadOid !== null
    && (!isObject(pr.head) || pr.head.sha !== expectedLiveHeadOid)
  ) {
    throw new Error(`${missionId} live PR head must equal the recorded publication head`);
  }
  if (
    proofCommitOid !== null
    && expectedLiveHeadOid !== null
    && expectedLiveHeadOid !== proofCommitOid
  ) {
    const commitOids = await fetchPullRequestCommitOids({ request, url: commitsUrl });
    if (!commitOids.includes(proofCommitOid)) {
      throw new Error(`${missionId} selected factory proof commit is not contained in the live PR`);
    }
  }

  let publicationAction = 'opened';
  if (verificationWindow !== null) {
    const startedAt = Date.parse(isoTime(
      verificationWindow.startedAt,
      `${missionId} proof verification_started_at`,
    ));
    const finishedAt = Date.parse(isoTime(
      verificationWindow.finishedAt,
      `${missionId} proof verification_finished_at`,
    ));
    const createdAt = Date.parse(isoTime(pr.created_at, `${missionId} GitHub PR created_at`));
    if (finishedAt < startedAt) {
      throw new Error(`${missionId} proof verification_finished_at precedes verification_started_at`);
    }
    if (finishedAt < createdAt) {
      publicationAction = 'opened';
    } else if (startedAt >= createdAt + 1_000) {
      // GitHub exposes created_at at whole-second precision. Requiring the proof
      // window to start after that second makes the amendment classification exact.
      publicationAction = 'updated';
    } else {
      throw new Error(`${missionId} verification timing overlaps PR creation and cannot prove initial or amendment wording`);
    }
  }
  ...
  if (requireBodyLink) {
    ...
    const acceptedStates = allowMergedOpenBlock && publication.state === 'merged'
      ? ['open', 'merged']
      : [publication.state];
    ...
    if (publicationAction === 'updated') {
      const expectedBlock = matchingBlocks[0].block;
      const finishedAt = Date.parse(verificationWindow.finishedAt);
      const edits = await fetchBodyEdits({ request, owner, repository, prNumber });
      const matchingEdits = edits.filter(({ body }) => body.includes(expectedBlock));
      if (matchingEdits.length === 0) {
        throw new Error(`${missionId} GitHub PR edit history does not contain the exact amendment block`);
      }
      if (matchingEdits.some(({ editedAt }) => Date.parse(editedAt) <= finishedAt)) {
        throw new Error(`${missionId} exact amendment block appeared before verification finished`);
      }
    }
...
async function factoryDisclosureRecord({ factoryReceiptsDir, entryName, policy }) {
  ...
  if (blockVersion === 2) {
    if (typeof proof.base_oid !== 'string' || !OID_PATTERN.test(proof.base_oid)) {
      throw new TypeError(`${missionId} proof base_oid must be a full commit OID`);
    }
    if (proof.base_oid === proof.commit_oid) {
      throw new TypeError(`${missionId} proof base_oid must differ from commit_oid`);
    }
    const startedAt = Date.parse(isoTime(
      proof.verification_started_at,
      `${missionId} proof verification_started_at`,
    ));
    const finishedAt = Date.parse(isoTime(
      proof.verification_finished_at,
      `${missionId} proof verification_finished_at`,
    ));
    if (finishedAt < startedAt) {
      throw new TypeError(`${missionId} proof verification_finished_at precedes verification_started_at`);
    }
    if (typeof publication.pr_head_oid !== 'string' || !OID_PATTERN.test(publication.pr_head_oid)) {
      throw new TypeError(`${missionId} factory publication pr_head_oid must be a full commit OID`);
    }
    if (publicationState !== 'merged' && publication.pr_head_oid !== proof.commit_oid) {
      throw new TypeError(`${missionId} factory publication pr_head_oid must equal the selected proof commit`);
    }
  }
  return {
    missionId,
...
async function auditFactoryDisclosureRecord({ record, policy, request }) {
  ...
  const remote = await auditRemote({
    ...
    verificationWindow: record.blockVersion === 2 ? {
      startedAt: record.proof.verification_started_at,
      finishedAt: record.proof.verification_finished_at,
    } : null,
    expectedLiveHeadOid: record.blockVersion === 2 ? record.publication.pr_head_oid : null,
    proofCommitOid: record.blockVersion === 2 ? record.proof.commit_oid : null,
  });
  return {
    remote,
    report: {
...
export async function syncFactoryDisclosure({
  ...
  const updated = upsertDisclosureBlock(remote.body, {
    ...
    publicationAction: remote.publicationAction,
  });
  if (!updated.changed) {
...
async function githubGraphqlGetFallback({ fetchImpl, token, url }) {
  ...
  if (pullMatch) {
    const result = await queryGraphql(
      'query($owner:String!,$name:String!,$number:Int!){repository(owner:$owner,name:$name){pullRequest(number:$number){number url body createdAt headRefOid}}}',
      variables,
    );
